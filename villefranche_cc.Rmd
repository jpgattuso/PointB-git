...
# **Material and methods** {.tabset .tabset-pills}
...
# **Atmospheric / Seawater pCO2** {.tabset .tabset-pills}
...
CO2_pr_i <- CO2_pr_i %>%
  dplyr::mutate(year=lubridate::year(dt), monthd = lubridate::month(dt), month = lubridate::month(dt, label = TRUE, abbr = FALSE))
# # add seawater pCO2
...
colnames(par_10m)  <- c("datetime", "raw_value", "int_light")
...
colnames(par_20m)  <- c("datetime", "raw_value", "int_light")
```
...
colnames(par_30m)  <- c("datetime", "raw_value", "int_light")
```
...
![](/https://docs.google.com/spreadsheets/d/e/2PACX-1vSmth08Q-Tw1RFznKeStSMJkKeFgYZac9WIR3ETLYWkznouAD6A-1n2uza1YgotXQ/pub?gid=798409504&single=true&output=pdf)

test

<embed src="https://docs.google.com/spreadsheets/d/e/2PACX-1vSmth08Q-Tw1RFznKeStSMJkKeFgYZac9WIR3ETLYWkznouAD6A-1n2uza1YgotXQ/pub?gid=798409504&single=true&output=pdf" width="500" height="375" 
...
```{r plot ctd_hydro 1m Temp, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Temp 1m
...
```{r plot ctd_hydro 5m Temp, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Temp 5m
...
```{r plot ctd_hydro 50m Temp, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Temp 50m
...
### **1 m**
```{r plot ctd_hydro 1m Sal, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Sal 1m
...
```{r plot ctd_hydro 5m Sal, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Sal
...
```{r plot ctd_hydro 50m Sal, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Sal
...
```{r plot ctd_hydro 1m Oxy, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Oxy umolkg 1m
...
```{r plot ctd_hydro 5m Oxy, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Oxy umolkg 5m
...
```{r plot ctd_hydro 50m Oxy, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
## Oxy umolkg 50m
...
- SOMLIT data are filtered in the "Rade hydro, SOMLIT" section above. Data from B station (not B+) and from 1 m to 3 m were selected. Time was not available at the beginning. We decided to add 09:00:00 as a local time. It was convert to UTC time in order to make the comparison with EOL data.
...
#combiine the 2 data frame by nearest time
...
#eol_ptb_1m
eol_ptb_temp_xts <- dplyr::select(combined1, datetime, mean_temp_rhB, temp_eol)
...
fit <- lmodel2(data = combined1, mean_temp_rhB ~ temp_eol, nperm = 99)
...
```{r eol pointb 5m Temp, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
...
eol_ptb_temp_xts <- dplyr::select(combined5, datetime, mean_temp_rhB, temp_eol)
...
###
titre <- "Temperature 5m: EOL vs Point B"
fit <- lmodel2(data = combined5, mean_temp_rhB ~ temp_eol, nperm = 99)
p_Tptb_Teol_5m <-  ggreg2(fit, "temp_eol", "mean_temp_rhB") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
  mytheme+labs(x="T EOL", y="T Point B")
...
eol_ptb_sal_xts <- dplyr::select(combined1, datetime, mean_sal_rhB, sal_eol)
...
###
titre <- "Salinity: EOL vs Point B 1m"
fit <- lmodel2(data = combined1, mean_sal_rhB ~ sal_eol, nperm = 99)
...
#plot
sal_rhB_eol_xts <- dplyr::select(combined5,datetime, mean_sal_rhB, sal_eol, sal_eol_rhB_detla)
...
fit <- lmodel2(data = combined5, mean_sal_rhB ~ sal_eol, nperm = 99)
p_Sptb_Seol5 <-  ggreg2(fit, "sal_eol", "mean_sal_rhB") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
  mytheme+labs(x="Sal EOL", y="Sal Point B")
...
# PLOT all oxyegen data as time series
eol_ptb_oxy_xts <- dplyr::select(combined1, datetime, mean_oxy_mll_rhB, oxy_eol, ptb_oxy_wink)
...
fit <- lmodel2(data = combined1, ptb_oxy_wink ~ mean_oxy_mll_rhB, nperm = 99)
