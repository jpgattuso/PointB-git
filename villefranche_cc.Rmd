...
dygraph(par_xts, ylab = "") %>%
  ...
  dyEvent(as.POSIXct("2021-02-17 11:00:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
...
#plot
sal_eol_xts <- dplyr::select(combined1,datetime, sal_eol_rhBplus_detla)
sal_eol_xts <- as.xts(sal_eol_xts, order.by=sal_eol_xts$datetime)

dygraph(sal_eol_xts,  main = "Delta Sal EOL / ptB+ 1m", ylab = "") %>%
...
p6_xts <- as.xts(p6_xts, order.by=p6_xts$datetime)

dygraph(p6_xts,  main = "p6, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("sal_eol_rhBplus_detla", drawPoints = TRUE, pointSize=2, label = "delta", color="black")%>%
dyOptions(
    drawGrid = TRUE,
    drawPoints = TRUE,
...
p6_plot <-  ggplot(data=p6, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p6, intercept =intercept_p6), linetype = 1, col="red") +
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p6_plot)
...
p7_xts <- as.xts(p7_xts, order.by=p7_xts$datetime)

dygraph(p7_xts,  main = "p7, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("sal_eol_rhBplus_detla", drawPoints = TRUE, pointSize=2, label = "delta", color="black")%>%
dyOptions(
    drawGrid = TRUE,
    drawPoints = TRUE,
...
p7_plot <-  ggplot(data=p7, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p7, intercept =intercept_p7), linetype = 1, col="red") +
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p7_plot)
...
Parameters from drifting periodes have been applied to same periodes in high frequency dataset.
