...
mean_eol <- eol %>%
  ...
  dplyr::summarise(
            ...
            mean_O2_eol = mean(O2, na.rm=TRUE)) %>%
  dplyr::rename(datetime = datetime_round) %>%
  dplyr::filter(P >=1 & P <= 3)


## PLOT 2 = 0 to 2 m + rounded time to hour
eol_xts <- dplyr::select(mean_eol, datetime_round, mean_sal_eol, mean_temp_eol, mean_O2_eol)
...
dygraph(eol_xts,  main = "EOL data: 0 to 3 m depth , rounded time to nearest hour", ylab = "") %>%
  ...
  dySeries(axis = "y2","mean_sal_eol", drawPoints = TRUE, pointSize=2, label = "Sal") %>%
    dySeries("mean_O2_eol", drawPoints = TRUE, pointSize=2, label = "oxy")%>%
 # dyLegend(show = "always", width = 600)%>%
  #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
dyOptions(
    drawGrid = TRUE,
...
```{r eol pointb comp 1st, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
...
eol_ptb_xts <- dplyr::select(eol_ptb, datetime, mean_temp_rhB, mean_temp_eol)
eol_ptb_xts <- as.xts(eol_ptb_xts, order.by=eol_ptb$datetime)

dygraph(eol_ptb_xts,  main = "Temperature comparaison: EOL - PTB", ylab = "") %>%
