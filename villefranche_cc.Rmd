...
spectro$datetime <- with_tz(spectro$datetime, "UTC")#  tidyr::unite("datetime", sampling_date, sampling_hour, sep=" ", remove =T) %>%  
 ...
 #ph_sf <- left_join(ph_sf, spectro_mean%>%dplyr::select(datetime,ph_s_sf_temp_insi, temp_field,salinity_field),by="datetime") 
  ph_sf <- left_join(ph_sf, spectro%>%dplyr::select(datetime,ph_s_sf_temp_insi, temp_field,salinity_field),by="datetime") 

## Add event periodes manually
## Events
sami_events <- c("2019-12-16 11:00:00", "2020-02-25 16:00:00","2020-06-17 08:00:00","2020-06-19 10:00:00","2020-07-15 10:00:00","2020-07-17 18:00:00", "2020-08-10 00:00:00", "2020-12-10 13:00:00","2020-12-17 09:00:00","2021-02-17 09:00:00","2021-03-17 12:00:00","2021-03-30 09:30:00","2021-04-22 12:00:00","2021-06-11 08:00:00","2021-07-20 07:00:00","2021-09-24 13:00:00","2021-10-19 09:00:00", "2021-11-26 08:00:00","2021-11-26 10:00:00")
...
## Oxy umolkg 50m
rh_oxymll_xts <- dplyr::select(rh_B_Bplus_50m_mean, date, mean_oxy_mll_rhBplus_B, mean_oxy_mll_rhBplus,mean_oxy_mll_rhB)
rh_oxymll_xts <- as.xts(rh_oxymll_xts, order.by=rh_oxymll_xts$date)
...
dygraph(rh_oxymll_xts,  main = "Oxygen (ml/l 50m Rade Hydro: B / B+", ylab = "") %>%
  ...
  dySeries("mean_oxy_mll_rhBplus", drawPoints = TRUE, pointSize=2, label = "Oxy_mll B+", color="red")%>%
    dySeries("mean_oxy_mll_rhBplus_B", drawPoints = TRUE, pointSize=2, label = "Oxy_mll B/B+", color="black")%>%
...
saveRDS(combined_50_B_Bplus_wink, file = paste0(path, "data/eol/rh_B_Bplus_50m_mean.rds"))
```
...
eol <- eol%>%
  dplyr::mutate(correction_sal = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ slope_p1 * unclass(datetime) + intercept_p1, 
                                       ...
                                       datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" ~ slope_p8 * unclass(datetime) + intercept_p8,TRUE ~ 0),
                slope_sal = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ slope_p1 , 
                                       ...
                                       datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" ~ slope_p8 ,TRUE ~ NA_real_),
                 intercept_sal = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ intercept_p1 , 
                                       ...
                                       datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" ~ intercept_p8 ,TRUE ~ NA_real_),
...
# Regression lineaire avec le temps
fit_wink <- lm(data = combined1, oxy_mll_eol_wink_detla ~ unclass(datetime) )
slope_wink <- fit_wink$coefficients[[2]]
intercept_wink <- fit_wink$coefficients[[1]]

lims <- as.POSIXct(strptime(c("2017-07-01 00:00:00","2021-05-30 23:00:00"), format = "%Y-%m-%d %H:%M:%S"))    
wink_plot <-  ggplot(data=combined1, aes(x=datetime, y=oxy_mll_eol_wink_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_wink, intercept =intercept_wink), linetype = 1, col="red") + mytheme+labs(y="delta oxy EOL/Wink", x="") + scale_x_datetime(date_breaks="3 months",  labels=date_format("%b %Y"), limits =lims) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(wink_plot)
...
combined1 <- combined1%>%
  dplyr::mutate(correction_oxy = case_when(datetime >"2017-07-11 00:00:00" & datetime < "2021-05-25 23:59:00" ~ slope_wink * unclass(datetime) + intercept_wink, 
                                          TRUE ~ NA_real_),
                slope_oxy = case_when(datetime >"2017-07-11 00:00:00" & datetime < "2021-05-25 23:59:00" ~ slope_wink, 
                                          TRUE ~ NA_real_),
                intercept_oxy = case_when(datetime >"2017-07-11 00:00:00" & datetime < "2021-05-25 23:59:00" ~  intercept_wink, 
                                          TRUE ~ NA_real_),
