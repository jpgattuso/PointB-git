...
eol_ptb_sal_xts <- as.xts(eol_ptb_sal_xts, order.by=eol_ptb$datetime)
...
eol_ptb_oxy_xts <- as.xts(eol_ptb_oxy_xts, order.by=eol_ptb$datetime)

dygraph(eol_ptb_oxy_xts,  main = "Salinity comparaison: EOL - PTB", ylab = "") %>%
    dySeries("mean_oxy_rhB", drawPoints = TRUE, pointSize=4, label = "Sal PTB", color ="blue") %>%
dySeries("mean_oxy_eol", drawPoints = TRUE, pointSize=2, label = "Sal EOL", color ="red")%>%
 # dyLegend(show = "always", width = 600)%>%
  #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
dyOptions(
    drawGrid = TRUE,
