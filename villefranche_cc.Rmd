...
if (Sys.getenv("LOGNAME") == "gattuso") {
  ...
  path_sami = "../../pCloud\ Sync/Documents/experiments/exp176_sami/sami_data/sami_processed/Salinity_corrected/PointB/"
}
if (Sys.getenv("LOGNAME") == "samir") {
  path = "../../pCloud Sync/exp153_carbonates_point_B/"
  path_sami = "../../pCloud\ Sync/exp176_sami/sami_data/sami_processed/Salinity_corrected/PointB/"
}
...
typ <- c("ccccdddddfddfdffdfdffdfdfdfdfdf") # column type
z <-  read_delim(
...
means_month0m <- dplyr::ungroup(mean0m) %>%
...
means_month0m$monthd <-
  lubridate::month(means_month0m$monthd, label = TRUE, abbr = FALSE) # month name for table
...
# prepare data 50m
tmp <- dplyr::ungroup(mean50m) %>%
  dplyr::mutate(month = lubridate::month(sampling_date, label = TRUE, abb = TRUE)) %>%
  dplyr::select(
  month,
  mean_temperature,
  mean_salinity,
...
means_month50m <- dplyr::ungroup(mean50m) %>%
  dplyr::group_by(monthd) %>%
  dplyr::summarise(
  Salinity = mean(mean_salinity, na.rm = TRUE),
  Salinity_sd = sd(mean_salinity, na.rm = TRUE),
...
means_month50m$monthd <-
  lubridate::month(means_month50m$monthd, label = TRUE, abbr = FALSE) # month name for table
...
# Calculate anomalies
ano_month0m <- dplyr::left_join(dplyr::ungroup(mean0m), tmp, by = "monthd") %>%
  dplyr::mutate(
...
# Calculate anomalies
ano_month50m <- dplyr::left_join(dplyr::ungroup(mean50m), tmp, by = "monthd") %>%
  dplyr::mutate(
  Salinity_ano = mean_salinity - Salinity_month,
  Temperature_ano = mean_temperature - Temperature_month,
  dic_ano = mean_dic - dic_month,
...
mean0m_month <- mean0m %>%
  dplyr::group_by(year, monthd) %>%
  dplyr::summarise(Temperature = mean(mean_temperature, na.rm=TRUE),
...
#Prepare dataframe
monthly_rate_AT_CT <- 
  cbind(dplyr::rename(select(tab_at_month, Slope), AT=Slope),
        dplyr::rename(select(tab_ct_month, Slope), CT=Slope)
) %>%
...
mean50m_month <- mean50m %>%
  ...
  dplyr::summarise(Temperature = mean(mean_temperature, na.rm=TRUE),
            ...
            pCO2 = mean(mean_pCO2, na.rm=TRUE),
            ct = mean(mean_dic, na.rm=TRUE)) %>%
  dplyr::mutate(date = ymd(paste(year,monthd,"01")))
...
monthly_rate_AT_CT_50m <- 
  cbind(dplyr::rename(dplyr::select(tab_at_month_50m, Slope), AT=Slope),
        dplyr::rename(dplyr::select(tab_ct_month_50m, Slope), CT=Slope)
) %>%
  dplyr::mutate(month=factor(month.abb, as.character(month.abb))) %>%
  gather(key, value, -month) %>%
  dplyr::mutate(se=c(tab_at_month[,2], tab_ct_month[,2])) %>%
  dplyr::mutate(p=c(tab_at_month[,3], tab_ct_month[,3])) %>%
  dplyr::mutate(significance=ifelse(p<=0.05, "*", ""))
...
# Long series
continuous_discrete <- read_csv(file = paste0(path,"data/seafet/continuous_discrete.csv"), col_types = cols(pHcalc_ptB = "d", pHspec_Tptb="d", T_seaF = "d", T_int ="d"), col_names = TRUE)
...
plot_continuous_discrete_pH  <- ggplot(data = dplyr::filter(continuous_discrete,!is.na(pHT_sfint))) +
  geom_vline(xintercept = deploy, colour="grey") + 
  geom_point(aes(x = date, y = pHT_sfint), size = 0.1) +
  geom_point(data = dplyr::filter(continuous_discrete, !is.na(pHspec_Tptb)),
...
plot_continuous_discrete_T  <- ggplot(data = dplyr::filter(continuous_discrete,!is.na(T_seaF))) +
  geom_vline(xintercept = deploy, colour="grey") + 
...
dat <- dplyr::mutate(continuous_discrete, diff_pH = pHspec_Tptb - pHT_sfint)
...
range <- tbl_df(
  dat %>%
  dplyr::group_by(date=as.Date(date), month=month(date, label=TRUE), year=year(date)) %>%
  dplyr::summarize(pH_range = max(pHT_sfint) - min(pHT_sfint))
)
...
range_month <-
  range %>%
  dplyr::group_by(Month = month(date, label=TRUE, abbr=FALSE)) %>%
  dplyr::summarize(pH_range_month = mean(pH_range, na.rm=TRUE))
# add chla in range_month
range_month <- dplyr::inner_join(range_month, dplyr::select(means_month0m, Month, Temperature, Chla), by="Month")
...
range_year_week <-
  ...
  dplyr::summarize(pH_range_year_week = mean(pH_range, na.rm=TRUE))
# add year_month to weekly data
tmp <- dplyr::ungroup(mean0m) %>%
  dplyr::mutate(year_week=paste0(year,"-", week(sampling_date))) %>%
  dplyr::select(sampling_date, mean_Chla, year_week)
# add chla in range_month
range_year_week <- dplyr::inner_join(range_year_week, tmp, by="year_week")
...
kable(pander(summary(reg_Chla)), digits = c(5,5,5,3,3), caption="Regressions on values monthly means of pH range as a function of Chla")
```
...
Atmospheric concentration of CO2 measured at Plateau Rosa from 1993 to 2018 were downloaded from
...
# the discrete_continuous file is already read in chunk "Daily and seasonal changes"

#round time into nearest hour
continuous_discrete$datetime_round <-round_date(continuous_discrete$datetime,unit = "hour" )
```
## point B
```{r point b, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
y <- read.table("../../pCloud Sync/exp153_carbonates_point_B/data/Data_DIC.csv", header=TRUE, sep=",", dec=".", as.is=TRUE)
```
...
colnames(sami)  <- c("datetime","ph_sami", "ph_error_sami", "temp_int_sami", "temp_ext_sami","flag_sami")

# add deployment period = Fisrt, Add manualy the deployement number in the title file and take last digit in the title file
sami$deployment_sami <- as.numeric(substr(str_extract(file, "-[0-9][0-9]"),2,3))
...
save(sami, file = paste0(path, "data/sami/sami_all_ptb.csv"))
...
rhB_surf_mean <- radehydro_B_surf %>%
  dplyr::group_by(date) %>%
...
# B+
radehydro_Bplus_surf <- dat %>%
  dplyr::select(station, date , depth, salinity, temperature, oxygen_mll, oxygen_umolkg) %>%
  dplyr::filter( (depth >= 1 & depth <= 3))%>%
  dplyr::filter( station == "B+")

rhBplus_surf_mean <- radehydro_Bplus_surf %>%
  dplyr::group_by(date) %>%
...
rh_B_Bplus_mean <- full_join(rhB_surf_mean, rhBplus_surf_mean, by=NULL)
rh_B_Bplus_mean$date <- as.POSIXct(rh_B_Bplus_mean$date)

# put 09:00:00 as time (local time)
rh_B_Bplus_mean <- rh_B_Bplus_mean %>%
  dplyr::mutate(time = " 09:00:00") %>%
  dplyr::mutate(datetime = paste0(date, time)) 
rh_B_Bplus_mean$datetime <- ymd_hms(rh_B_Bplus_mean$datetime, tz="Europe/Paris")

# convert local time to UTC time
rh_B_Bplus_mean$datetime <- with_tz(rh_B_Bplus_mean$datetime, "UTC")

```

### Time series plots

```{r plot ctd_hydro, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
# Temp
rh_temp_xts <- dplyr::select(rh_B_Bplus_mean, date, mean_temp_rhBplus, mean_temp_rhB)
rh_temp_xts <- as.xts(rh_temp_xts, order.by=rh_temp_xts$date)

dygraph(rh_temp_xts,  main = "Temperature comparaison Rade Hydro: B / B+", ylab = "") %>%
...
# Sal
rh_sal_xts <- dplyr::select(rh_B_Bplus_mean, date, mean_sal_rhBplus, mean_sal_rhB)
rh_sal_xts <- as.xts(rh_sal_xts, order.by=rh_sal_xts$date)

dygraph(rh_sal_xts,  main = "Salinity comparaison Rade Hydro: B / B+", ylab = "") %>%
...
# Oxy ml/l
rh_oxymll_xts <- dplyr::select(rh_B_Bplus_mean, date, mean_oxy_mll_rhBplus, mean_oxy_mll_rhB)
rh_oxymll_xts <- as.xts(rh_oxymll_xts, order.by=rh_oxymll_xts$date)

dygraph(rh_oxymll_xts,  main = "Oxygen (ml/l) comparaison Rade Hydro: B / B+", ylab = "") %>%
...
# Oxy umolkg
rh_oxyumolkg_xts <- dplyr::select(rh_B_Bplus_mean, date, mean_oxy_umolkg_rhBplus, mean_oxy_umolkg_rhB)
rh_oxyumolkg_xts <- as.xts(rh_oxyumolkg_xts, order.by=rh_oxyumolkg_xts$date)

dygraph(rh_oxyumolkg_xts,  main = "Oxygen (umol/kg) comparaison Rade Hydro: B / B+", ylab = "") %>%
...
```{r property plot ctd_hydro, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
# temp
titre <- "Temperature Rade Hydro: B vs B+"
fit <- lmodel2(data = rh_B_Bplus_mean, mean_temp_rhBplus ~ mean_temp_rhB, nperm = 99)
...
# sal
titre <- "Salinity Rade Hydro: B vs B+"
fit <- lmodel2(data = rh_B_Bplus_mean, mean_sal_rhBplus ~ mean_sal_rhB, nperm = 99)
...
# Oxy mll
titre <- "Oxygen (ml/l) Rade Hydro: B vs B+"
fit <- lmodel2(data = rh_B_Bplus_mean, mean_oxy_mll_rhBplus ~ mean_oxy_mll_rhB, nperm = 99)
...
# Oxy umolkg
titre <- "Oxygen (umol/kg) Rade Hydro: B vs B+"
fit <- lmodel2(data = rh_B_Bplus_mean, mean_oxy_umolkg_rhBplus ~ mean_oxy_umolkg_rhB, nperm = 99)
...
temperature <- left_join(mean_eol%>%select(datetime_round, mean_temp_eol), continuous_discrete%>%select(datetime_round, T_ptb), by="datetime_round")

temperature_xts <- dplyr::select(temperature, datetime_round, T_ptb, mean_temp_eol)
temperature_xts <- as.xts(temperature_xts, order.by=temperature_xts$datetime_round)
...
```{r eol pointb comp Temp 2nd, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
...
eol_ptb <- left_join(rh_B_Bplus_mean, mean_eol , by="datetime")

eol_ptb_temp_xts <- dplyr::select(eol_ptb, datetime, mean_temp_rhB, mean_temp_eol)
eol_ptb_temp_xts <- as.xts(eol_ptb_temp_xts, order.by=eol_ptb$datetime)

dygraph(eol_ptb_temp_xts,  main = "Temperature comparaison: EOL - PTB", ylab = "") %>%
    dySeries("mean_temp_rhB", drawPoints = TRUE, pointSize=4, label = "Temp PTB", color ="blue") %>%
dySeries("mean_temp_eol", drawPoints = TRUE, pointSize=2, label = "Temp EOL", color ="red")%>%
 # dyLegend(show = "always", width = 600)%>%
  #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
dyOptions(
    drawGrid = TRUE,
...
###
titre <- "Temperature: EOL vs Point B"
fit <- lmodel2(data = eol_ptb, mean_temp_rhB ~ mean_temp_eol, nperm = 99)
p_Tptb_Teol <-  ggreg2(fit, "mean_temp_eol", "mean_temp_rhB") +
...
```
### Salinity
```{r eol pointb comp Sal, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}

eol_ptb_sal_xts <- dplyr::select(eol_ptb, datetime, mean_sal_rhB, mean_sal_eol)
...
dySeries("mean_sal_eol", drawPoints = TRUE, pointSize=2, label = "Sal EOL", color ="red")%>%
 # dyLegend(show = "always", width = 600)%>%
  #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
dyOptions(
    drawGrid = TRUE,
...
fit <- lmodel2(data = eol_ptb, mean_sal_rhB ~ mean_sal_eol, nperm = 99)
p_Sptb_Seol <-  ggreg2(fit, "mean_sal_eol", "mean_sal_rhB") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
...
```
### Oxygen
```{r eol pointb comp Oxy, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}

eol_ptb_oxy_xts <- dplyr::select(eol_ptb, datetime, mean_oxy_mll_rhB, mean_oxy_eol)
eol_ptb_oxy_xts <- as.xts(eol_ptb_oxy_xts, order.by=eol_ptb_oxy_xts$datetime)

dygraph(eol_ptb_oxy_xts,  main = "Oxygen (ml/l) comparaison: EOL - PTB", ylab = "") %>%
dySeries("mean_oxy_mll_rhB", drawPoints = TRUE, pointSize=4, label = "Oxy PTB", color ="blue") %>%
dySeries("mean_O2_eol", drawPoints = TRUE, pointSize=2, label = "Oxy EOL", color ="red")%>%
 # dyLegend(show = "always", width = 600)%>%
  #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
dyOptions(
    drawGrid = TRUE,
...
###
titre <- "Salinity: EOL vs Point B"
fit <- lmodel2(data = eol_ptb, mean_oxy_mll_rhB ~ mean_O2_eol, nperm = 99)
p_Oxyptb_Oxyeol <-  ggreg2(fit, "mean_O2_eol", "mean_oxy_mll_rhB") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
  mytheme+labs(x="Sal EOL", y="Sal Point B")
