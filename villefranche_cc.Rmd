...
#### Temperature all dataset  {.tabset} 
...
# plot 2008
dat0m2008 <- dat0m %>% dplyr::filter(year ==2008)
fit <- lm(data = dat0m2008, ta ~ salinity)
p_ta_sal_lm2008 <- ggplot(dat0m2008,aes(x=salinity, y= ta, label = monthd))+
  geom_text( aes(color=factor(monthd))) +
  scale_color_discrete(guide="none")+
    labs(x="Salinity 2008", y="TA 2008", color = "Months",title = "Only 2008")  +
    stat_smooth(method = "lm", col = "black") +
    labs(title = paste("", "\nAdj R2 = ",signif(summary(fit)$adj.r.squared, 5),
                       ";\nIntercept =",signif(fit$coef[[1]],5 ),
                       ";\nSlope =",signif(fit$coef[[2]], 5),
  ...
  theme_bw() + theme(aspect.ratio=1,
        ...
        plot.margin = unit(c(0.5,1,1,1), "cm"),
        plot.title = element_text(size=7, hjust=0.05,margin = margin(l = 10, b = -70)))
print(p_ta_sal_lm2008)

# plot 2018
dat0m2018 <- dat0m %>% dplyr::filter(year ==2018)
fit <- lm(data = dat0m2018, ta ~ salinity)
p_ta_sal_lm2018 <- ggplot(dat0m2018,aes(x=salinity, y= ta, label = monthd))+
  geom_text( aes(color=factor(monthd))) +
  scale_color_discrete(guide="none")+
    labs(x="Salinity 2018", y="TA 2018", color = "Months",title = "Only 2018")  +
    stat_smooth(method = "lm", col = "black") +
    labs(title = paste("", "\nAdj R2 = ",signif(summary(fit)$adj.r.squared, 5),
                       ";\nIntercept =",signif(fit$coef[[1]],5 ),
                       ";\nSlope =",signif(fit$coef[[2]], 5),
  ...
  theme_bw() + theme(aspect.ratio=1,
        ...
        plot.margin = unit(c(0.5,1,1,1), "cm"),
        plot.title = element_text(size=7, hjust=0.05,margin = margin(l = 10, b = -70)))
print(p_ta_sal_lm2018)
# plot 2019
dat0m2019 <- dat0m %>% dplyr::filter(year ==2019)
fit <- lm(data = dat0m2019, ta ~ salinity)
p_ta_sal_lm2019 <- ggplot(dat0m2019,aes(x=salinity, y= ta, label = monthd))+
  geom_text( aes(color=factor(monthd))) +
  scale_color_discrete(guide="none")+
    labs(x="Salinity 2019", y="TA 2019", color = "Months",title = "Only 2019")  +
    stat_smooth(method = "lm", col = "black") +
    labs(title = paste("", "\nAdj R2 = ",signif(summary(fit)$adj.r.squared, 5),
                       ";\nIntercept =",signif(fit$coef[[1]],5 ),
                       ";\nSlope =",signif(fit$coef[[2]], 5),
  ...
  theme_bw() + theme(aspect.ratio=1,
        ...
        plot.margin = unit(c(0.5,1,1,1), "cm"),
        plot.title = element_text(size=7, hjust=0.05,margin = margin(l = 10, b = -70)))
print(p_ta_sal_lm2019)
```

```{r Total alkalinity Point B 50 m, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
# AT=f(S) at 50 m 
ta_sal_xts50m <- dplyr::select(dat50m,sampling_date, salinity, ta)
ta_sal_xts50m <- as.xts(ta_sal_xts50m, order.by=ta_sal_xts50m$sampling_date)

dygraph(ta_sal_xts50m,  main = "TA / Salinity 50 m SOMLIT", ylab = "") %>%
    dySeries("salinity", drawPoints = TRUE, pointSize=4, label = "Sal", color ="blue") %>%
   dySeries("ta", drawPoints = TRUE, pointSize=2, label = "TA", color="red", axis="y2")%>%
 # dyLegend(show = "always", width = 600)%>%
  #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
...
#plot regression lm
fit <- lm(data = dat50m, ta ~ salinity)
p_ta_sal_lm50m <- ggplot(dat50m,aes(x=salinity, y= ta, label = monthd))+
  geom_text( aes(color=factor(monthd))) +
  scale_color_discrete(guide="none")+
    labs(x="Salinity ", y="TA", color = "Months")  +
    stat_smooth(method = "lm", col = "black") +
