...
eol_ptb_1m <- left_join(rh_B_Bplus_1m_mean, mean_eol , by="datetime")

eol_ptb_temp_xts <- dplyr::select(eol_ptb_1m, datetime, mean_temp_rhB, mean_temp_eol)
...
fit <- lmodel2(data = eol_ptb_1m, mean_temp_rhB ~ mean_temp_eol, nperm = 99)
...
eol_ptb_sal_xts <- dplyr::select(eol_ptb_1m, datetime, mean_sal_rhB, mean_sal_eol)
...
fit <- lmodel2(data = eol_ptb_1m, mean_sal_rhB ~ mean_sal_eol, nperm = 99)
...
eol_ptb_oxy_xts <- dplyr::select(eol_ptb_1m, datetime, mean_oxy_mll_rhB, mean_oxy_eol, ptb_oxy_wink)
...
fit <- lmodel2(data = eol_ptb_1m, mean_oxy_mll_rhB ~ mean_oxy_eol, nperm = 99)
...
fit <- lmodel2(data = eol_ptb_1m, ptb_oxy_wink ~ mean_oxy_eol, nperm = 99)
p_Oxyptbwink_Oxyeol <-  ggreg2(fit, "mean_oxy_eol", "ptb_oxy_wink") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
