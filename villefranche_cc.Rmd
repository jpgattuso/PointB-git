...
means_depth <- dat %>%
  dplyr::group_by(depth, sampling_date) %>%
  dplyr::summarise(
...
# prepare data 0m
tmp <- ungroup(mean0m) %>%
  dplyr::mutate(month = month(sampling_date, label = TRUE, abb = TRUE)) %>%
  dplyr::select(
  month,
  mean_temperature,
  mean_salinity,
...
#Prepare dataframe
monthly_rate_AT_CT <- 
  cbind(dplyr::rename(dplyr::select(tab_at_month, Slope), AT=Slope),
        dplyr::rename(dplyr::select(tab_ct_month, Slope), CT=Slope)
) %>%
...
monthly_rate_AT_CT_50m <- 
  cbind(dplyr::rename(select(tab_at_month_50m, Slope), AT=Slope),
        dplyr::rename(select(tab_ct_month_50m, Slope), CT=Slope)
) %>%
  dplyr::mutate(month=factor(month.abb, as.character(month.abb))) %>%
  gather(key, value, -month) %>%
  dplyr::mutate(se=c(tab_at_month[,2], tab_ct_month[,2])) %>%
...
CO2_pr <- tbl_df(
  read_csv(file=paste0(path,"/data/CO2_data_PlateauRosa_daily_1993-2018_R.txt"), comment="#")
#, col_types = typ)
#read_csv(file=paste0(path,"/data/CO2_data_PlateauRosa_daily_1993-2015_R.csv"), col_names=TRUE, col_types = typ, skip=7)
) %>%
unite(col="dt", year, month, day, sep = "-" , remove = FALSE)%>%
dplyr::rename(atm_CO2= value) %>%
dplyr::mutate(dt= ymd(dt),
              atm_CO2 = case_when(atm_CO2 <= -999 ~ NA_real_ ,TRUE ~ atm_CO2)) %>%
# dplyr::mutate(year=year(dt)) %>%
dplyr::filter(dt >= "2007-01-01")

NAs_by_year <- dplyr::summarise(group_by(CO2_pr, year), NAs=sum(is.na(atm_CO2))) # N NAs per year
...
dygraph(sf_xts, ylab = "") %>%
  ...
  dySeries("pHcalc_ptB", drawPoints = TRUE, pointSize=2, label = "pH calc (T ptB)") %>%
...
for (file in file_list){ 
  ii <- ii + 1
  tmp <- read.table(paste(path_sami, file, sep = ""), header = F, dec = ".", as.is = T, sep = "\t", skip = 4, fill = TRUE)
...
eol$datetime <-round_date(eol$datetime,unit = "hour" )
=======
>>>>>>> work1
...
eol_xts <- dplyr::select(mean_eol, datetime, mean_sal_eol, mean_temp_eol, mean_oxy_eol)
eol_xts <- as.xts(eol_xts, order.by=eol_xts$datetime)

dygraph(eol_xts,  main = "EOL data: 1 to 3 m depth , mean by day", ylab = "") %>%
...
### Time series plots (for Point B / B+)
...
dplyr::rename(date = DATE, 
              ptb_oxy_wink_dup= "O (Oxygène)")) 
winkler$date <- as.POSIXct(winkler$date)
winkler$ptb_oxy_wink_dup <- as.numeric(as.character(winkler$ptb_oxy_wink_dup))

# Average ptb_oxy_wink_dup data that are in duplicate (raw data)
wink <- winkler %>%
  dplyr::group_by(date) %>%
  dplyr::select(date,ptb_oxy_wink_dup,qO )%>%
  dplyr::filter(qO == 2  |qO ==6)%>%
  dplyr::summarise(ptb_oxy_wink = mean(ptb_oxy_wink_dup, na.rm = T)) 

# Adding the Winkler PointB data perweek to eol_ptb.

eol_ptb <- left_join(eol_ptb, wink , by="date")


# PLOT all oxyegen data as time series
eol_ptb_oxy_xts <- dplyr::select(eol_ptb, datetime, mean_oxy_mll_rhB, mean_oxy_eol, ptb_oxy_wink)
...
dySeries("mean_oxy_eol", drawPoints = TRUE, pointSize=2, label = "Oxy EOL", color ="red")%>%
...
dygraph(eol_ptb_oxy_xts,  main = "Oxygen (ml/l) comparaison: EOL - PTB (CTD + Winkler)", ylab = "") %>%
dySeries("mean_oxy_mll_rhB", drawPoints = TRUE, pointSize=4, label = "Oxy PTB", color ="blue") %>%
dySeries("mean_oxy_eol", drawPoints = TRUE, pointSize=2, label = "Oxy EOL", color ="red")%>%
 # dyLegend(show = "always", width = 600)%>%
  #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
...
###
titre <- "Salinity: EOL vs Point B"
fit <- lmodel2(data = eol_ptb, mean_oxy_mll_rhB ~ mean_oxy_eol, nperm = 99)
p_Oxyptb_Oxyeol <-  ggreg2(fit, "mean_oxy_eol", "mean_oxy_mll_rhB") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
  mytheme+labs(x="Sal EOL", y="Sal Point B")
