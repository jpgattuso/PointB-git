...
par_periode <- c("2020-10-29 12:20:00", "2020-12-09 08:00:00","2020-12-17 10:00:00","2021-03-16 11:00:00", "2021-05-25 06:00:00")
...
dygraph(par_xts, ylab = "") %>%
  ...
  dySeries("int_light_30", drawPoints = TRUE, pointSize=3, color= "dodgerblue4", label = "PAR 30m") %>%
    dyEvent(as.POSIXct("2020-12-28 09:10:00"), "par_10m stops", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
  dyEvent(as.POSIXct("2021-02-17 11:00:00"), "cleanning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
    dyEvent(as.POSIXct("2021-03-16 10:00:00"), "cleanning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
  dyEvent(as.POSIXct("2021-03-30 09:50:00"), "cleanning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
dyShading(from = par_periode[1], to = par_periode[2], color = "#CCCCCC") %>%
...
kbl(par_log)%>%
  ...
  row_spec(c(1:12, 25:36), bold = T, color = "white", background = "#97C7FA")
...
## Events
sami_events <- c("2019-12-16 11:00:00", "2020-02-25 16:00:00","2020-06-17 08:00:00","2020-06-19 10:00:00","2020-07-15 10:00:00","2020-07-17 18:00:00", "2020-08-10 00:00:00", "2020-12-10 13:00:00","2020-12-17 09:00:00","2021-02-17 09:00:00","2021-03-17 12:00:00","2021-03-30 09:30:00","2021-04-22 10:30:00","2021-06-11 08:00:00")
...
# periodes 
sal_periode <- c("2014-08-19 00:00:00","2015-06-16 00:00:00","2015-06-22 00:00:00", "2016-03-22 23:59:59","2017-01-10 00:00:00","2017-07-04 00:00:00", "2017-10-10 00:00:00", "2019-03-19 00:00:00","2019-07-09 00:00:00", "2020-03-10 00:00:00", "2020-04-23 00:00:00", "2020-11-17 00:00:00", "2020-11-24 00:00:00")
...
dyShading(from =  sal_periode[13], to= Sys.Date() , color = "#CCCCCC") %>%
...
Drifting periodes have been define according to the delta (drift) between sal Ptb+ and sal EOL.
...
#### Delta with -0.3/+0.3
oxy_eol_wink_xts <- dplyr::select(combined1%>%filter(sal_eol_rhBplus_detla > -0.3 & sal_eol_rhBplus_detla < 0.3 ),datetime, sal_eol_rhBplus_detla)
oxy_eol_wink_xts <- as.xts(oxy_eol_wink_xts, order.by=oxy_eol_wink_xts$datetime)

dygraph(oxy_eol_wink_xts,  main = "Delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("sal_eol_rhBplus_detla", drawPoints = TRUE, pointSize=2, label = "delta (PSU)", color="black")%>%
   dyLimit(0,strokePattern = "dashed", color = "black") %>% 
     dyLimit(0.3,strokePattern = "dashed", color = "red") %>% 
       dyLimit(-0.3,strokePattern = "dashed", color = "red") %>% 
...
##Plot
p1_xts <-p1%>%
  dplyr::select(datetime, sal_eol_rhBplus_detla)

p1_xts <- as.xts(p1_xts, order.by=p1_xts$datetime)

dygraph(p1_xts,  main = "p1, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
...
p1_plot <-  ggplot(data=p1, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p1, intercept =intercept_p1), linetype = 1, col="red") +
...
p2 <- combined1%>%
  dplyr::select(datetime, mean_sal_rhBplus, sal_eol,sal_eol_rhBplus_detla )%>%
    dplyr::filter(sal_eol_rhBplus_detla > -0.3 & sal_eol_rhBplus_detla < 0.3 )%>%
  dplyr::filter(datetime >"2015-06-22 00:00:00" & datetime <"2016-03-16 23:59:00" )

##Plot
p2_xts <-p2%>%
  dplyr::select(datetime, sal_eol_rhBplus_detla)

p2_xts <- as.xts(p2_xts, order.by=p2_xts$datetime)

dygraph(p2_xts,  main = "p2, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("sal_eol_rhBplus_detla", drawPoints = TRUE, pointSize=2, label = "delta", color="black")%>%
dyOptions(
    drawGrid = TRUE,
    drawPoints = TRUE,
...
p2_plot <-  ggplot(data=p2, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p2, intercept =intercept_p2), linetype = 1, col="red") +
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p2_plot)
...
p3_xts <- as.xts(p3_xts, order.by=p3_xts$datetime)

dygraph(p3_xts,  main = "p3, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("sal_eol_rhBplus_detla", drawPoints = TRUE, pointSize=2, label = "delta", color="black")%>%
dyOptions(
    drawGrid = TRUE,
    drawPoints = TRUE,
...
p3_plot <-  ggplot(data=p3, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p3, intercept =intercept_p3), linetype = 1, col="red") +
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p3_plot)
...
p4 <- combined1%>%
  dplyr::select(datetime, mean_sal_rhBplus, sal_eol,sal_eol_rhBplus_detla )%>%
    dplyr::filter(sal_eol_rhBplus_detla > -0.3 & sal_eol_rhBplus_detla < 0.3 )%>%
  dplyr::filter(datetime >"2017-11-07 00:00:00" & datetime <"2020-03-10 23:59:00" )

##Plot
p4_xts <-p4%>%
  dplyr::select(datetime, sal_eol_rhBplus_detla)

p4_xts <- as.xts(p4_xts, order.by=p4_xts$datetime)

dygraph(p4_xts,  main = "p4, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("sal_eol_rhBplus_detla", drawPoints = TRUE, pointSize=2, label = "delta", color="black")%>%
dyOptions(
    drawGrid = TRUE,
    drawPoints = TRUE,
...
p4_plot <-  ggplot(data=p4, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p4, intercept =intercept_p4), linetype = 1, col="red") +
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p4_plot)
...
p5 <- combined1%>%
  dplyr::select(datetime, mean_sal_rhBplus, sal_eol,sal_eol_rhBplus_detla )%>%
    dplyr::filter(sal_eol_rhBplus_detla > -0.3 & sal_eol_rhBplus_detla < 0.3 )%>%
  dplyr::filter(datetime >"2020-04-23 00:00:00" & datetime <"2020-11-17 23:59:00" )

##Plot
p5_xts <-p5%>%
  dplyr::select(datetime, sal_eol_rhBplus_detla)

p5_xts <- as.xts(p5_xts, order.by=p5_xts$datetime)

dygraph(p5_xts,  main = "p5, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("sal_eol_rhBplus_detla", drawPoints = TRUE, pointSize=2, label = "delta", color="black")%>%
dyOptions(
    drawGrid = TRUE,
    drawPoints = TRUE,
...
p5_plot <-  ggplot(data=p5, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p5, intercept =intercept_p5), linetype = 1, col="red") +
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p5_plot)
...
eol <- eol%>%
  dplyr::mutate(correction = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ slope_p1 * unclass(datetime) + intercept_p1, 
                                       datetime >"2015-06-22 00:00:00" & datetime <"2016-03-16 23:59:00" ~ slope_p2 * unclass(datetime) + intercept_p2,
                                       datetime >"2017-01-10 00:00:00" & datetime <"2017-07-04 23:59:00" ~ slope_p3 * unclass(datetime) + intercept_p3,
                                       datetime >"2017-11-07 00:00:00" & datetime <"2020-03-10 23:59:00" ~ slope_p4 * unclass(datetime) + intercept_p4,
                                       datetime >"2020-04-23 00:00:00" & datetime <"2020-11-17 23:59:00" ~ slope_p5 * unclass(datetime) + intercept_p5,
                                       TRUE ~ 0),
                sal_eol_corr = sal_eol + correction)
...
eol_ptb_oxy_xts <- dplyr::select(combined_1_B_Bplus_wink_eol, datetime, mean_oxy_mll_rhBplus, oxy_eol, ptb_oxy_wink, oxy_eol_calc)
eol_ptb_oxy_xts <- as.xts(eol_ptb_oxy_xts, order.by=eol_ptb_oxy_xts$datetime)

dygraph(eol_ptb_oxy_xts,  main = "Oxygen (ml/l) 1m: EOL - ptB+ - Winkler ptB)", ylab = "") %>%
dySeries("mean_oxy_mll_rhBplus", drawPoints = TRUE, pointSize=4, label = "Oxy ptB+", color ="blue") %>%
dySeries("oxy_eol", drawPoints = TRUE, pointSize=2, label = "Oxy EOL", color ="red")%>%
  dySeries("oxy_eol_calc", drawPoints = TRUE, pointSize=2, label = "Oxy EOL calc", color ="orange")%>%
