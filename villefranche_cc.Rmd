...
#make the mean with same date en rounded time + filter only surface data 1 to 3 m.
means_eol <- eol %>%
...
eol_xts <- dplyr::select(means_eol, datetime_round, mean_sal_eol, mean_temp_eol, mean_O2_eol)
...
## EOL - SOMLIT (Point B) comparaison

In this section, T, S and Oxy is compaired between EOL and SOMLIT data. 

EOL data are filtered in the
### Temperature
```{r eol pointb comp, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}

### 1st approach with pointB data from "continuous_discret" 
### 
# join PTB data from continuous_discrete to EOL data to compare
eol_ptb <- left_join(means_eol, continuous_discrete, by="datetime_round")

temperature <- left_join(means_eol%>%select(datetime_round, mean_temp_eol), continuous_discrete%>%select(datetime_round, T_ptb), by="datetime_round")
