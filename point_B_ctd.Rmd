...
There are 3 stations in the dataset: B, B+ and C. The temperature record starts at different dates : 
...
z <- dat %>% 
  ...
  dplyr::filter(station == 'B+' | (station == 'B' & date < "2004-12-21")) # eliminate B data when B+ starts

# add extra profiles of 2018
# See Profiles_sonde_supplémentaires_aout_2018_point B Samir.xlsx
supp1 <- read_excel(paste0(path,"data/", "Profiles_sonde_supplémentaires_aout_2018_pointB_Samir.xlsx"), sheet = 1) %>% 
  select(station, date, depth = PrdM, temperature = Potemp090C, salinity = Sal00) %>%
  mutate(date = ymd(date))
supp2 <- read_excel(paste0(path,"data/", "Profiles_sonde_supplémentaires_aout_2018_pointB_Samir.xlsx"), sheet = 2) %>% 
  select(station, date, depth = PrdM, temperature = Potemp090C, salinity = Sal00) %>%
  mutate(date = ymd(date))
...
In the present analysis, I combine the time-series at Point B and Point B+.
...
dat2 <- dat %>%
  add_row(station=rep("B",75), date = rep("1992-01-01", 75), depth = 1:75) %>% #to have full year 1992
  add_row(station=rep("B",75), date = rep("1992-02-01", 75), depth = 1:75) %>%
  add_row(station=rep("B",75), date = rep("1992-03-01", 75), depth = 1:75) %>%
  add_row(station=rep("B",75), date = rep("1992-04-01", 75), depth = 1:75) %>%
  ...
  add_row(station=rep("B",75), date = rep("2019-10-01", 75), depth = 1:75) %>%
  add_row(station=rep("B",75), date = rep("2019-11-01", 75), depth = 1:75) %>%
  add_row(station=rep("B",75), date = rep("2019-12-01", 75), depth = 1:75) %>%
...
temp_ts <- dat2 %>% # average temperature for the whole time series
...
plot_temp_ano <- ave_yr %>%
  ...
  geom_segment(aes(x=date, xend=date, y=0, yend=temp_ano, color=mycolor), size=1.3, alpha=0.9) +
  theme_light() +
  theme(
    legend.position = "none") +
  labs(x = "", y = "",
       title = "Monthly temperature anomaly in °C at Point B, Villefranche-sur-mer, NW Med.",
