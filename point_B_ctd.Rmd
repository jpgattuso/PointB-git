...
temp_ts <- dat2 %>% # average temperature for the whole time series
  ...
  summarize(temp_ts = mean(temperature, na.rm = TRUE)) %>%
...
# calculate monthly averages for each year
  ...
  group_by(year, month, range) %>%
    ...
    ave_yr <- left_join(temp_yr, temp_ts) %>%
        mutate(temp_ano = temp - temp_ts) #monthly anomalies for each year

 plot_ano <- ave_yr %>%
   ggplot() +
  geom_bar(aes(x = month, y = temp_ano, group = range, col = range), alpha = 0.2)
# calculate monthly anomalies for each year
#for (yr in c(1992:1993, 1995:2019)) { #no data in 1994, CTD lost
  #yr = 1992
#  new_var2 <- paste0("ano", as.character(yr))
#  ave_yr <- ave_yr %>%
#    mutate(ano = temp - temp_ts)
    #zz <- left_join(ave_all, z)
#}

#ave_all <- ave_all %>%
#  dplyr::mutate(range = forcats::fct_explicit_na(range, na_level = "(Missing)")) %>%
...
#   group_by(month, range) %>%
#   summarize(temp_2019 = mean(temperature, na.rm = TRUE), n2019 = n())
# ave_2019
