...
#                   na = c("", "NA"))
z <-  read_delim(
  paste0(path,"data/", "Data_DIC.csv"),col_types = cols(pH_ta = "d", temp_lab_eol="d", pH_s_eol_temp_lab ="d", qflag_pH_s_eol="d",
temp_lab_ptb ="d", pH_s_ptb_temp_lab ="d", qflag_pH_s_ptb="d"),
  delim = ",",
  na = c("NA", "999999")
)
...
if (j %in% c(1,4,7,10)) {p[[j]] <- p[[j]] + labs(y=expression(paste(italic(A)[T], " (",mu, "mol ", kg^-1,")")))}
 ...
 if (j %in% c(10,11,12)) {p[[j]] <- p[[j]] + labs(x = "Salinity")
