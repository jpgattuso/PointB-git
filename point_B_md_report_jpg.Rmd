...
# AT and CT, one removes  values flagged as 3
mean_at_dic <- z %>%
  dplyr::select(sampling_date, depth, ta_dup, qflag_ta, dic_dup, qflag_dic) %>% 
  dplyr::group_by(sampling_date, depth) %>%
  dplyr::mutate(ta_dup = ifelse(qflag_ta != 3, ta_dup, NA),
                dic_dup = ifelse(qflag_dic != 3, dic_dup, NA),
                ta = mean(ta_dup, na.rm = TRUE),
...
mean_sal_temp <- z %>%
  dplyr::select(sampling_date, depth, salinity_field, temp_field) %>% 
  dplyr::group_by(sampling_date, depth) %>%
...
mean_nuts <- z %>%
  ...
  dplyr::group_by(sampling_date, depth) %>%
  dplyr::mutate(NO2 = ifelse(qNO2 == 2, NO2, NA),
...
tmp <- z %>% 
  dplyr::select(sampling_date, depth, contains("eol"), contains("ptb")) %>% 
...
dat <- dplyr::full_join(dat1, eol_ptb, by = c("sampling_date", "depth")) %>% 
  dplyr::select(-dplyr::contains(c("eol", "ptb", "qflag", "qNO3", "qNO2", "qPO4", "qSIOH4"))) %>% 
    distinct()
# also add P and time variables, and rename nuts
means_depth <- dat %>%
  group_by(depth, sampling_date) %>%
  dplyr::summarise(
            P = mean(depth/10, na.rm=TRUE),
            Pt = mean(1e-6*PO4, na.rm=TRUE),
            Sit = mean(1e-6*SIOH4, na.rm=TRUE)
            ) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(
    year = year(sampling_date),
...
somlit <- means_depth %>% 
  dplyr::select(sampling_date, depth, salinity, temperature, dic, ta, pH_insi, Pt, Sit)
