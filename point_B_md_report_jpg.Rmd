...
ggreg2 <- function (fit, xdata, ydata, type = "MA") { # x and y are the names of the variables
...
slope <- reg$Slope
  ...
  geom_abline(aes(intercept = fit$regression.results[ty,2], slope = fit$regression.results[ty,3]),
              colour = "blue")  + 
...
# AT 50 m
# creat a new df to calculate the diff between duplicates AT and duplicates CT
 diff <- z %>%
   dplyr::filter(depth ==50) %>%
    dplyr::group_by(sampling_date) %>%
   dplyr::select(sampling_date, dic_dup, ta_dup)%>%   
   dplyr::mutate(at_diff = abs(ta_dup - lag(ta_dup)),
                 ct_diff = abs(dic_dup - lag(dic_dup)))
 # plot the differences between duplicates
at_diff_xts <- dplyr::select(diff, sampling_date, at_diff, ta_dup)
at_diff_xts <- as.xts(at_diff_xts, order.by = diff$sampling_date)
dygraph(at_diff_xts, main="Differences between AT duplicates 50 m", ylab="AT") %>%
    dySeries("at_diff", label = "AT delta", color ="red", strokeWidth=0, pointSize=4) %>%
    dySeries("ta_dup", label = "AT duplicates", color = "blue", strokeWidth = 0, pointSize = 2, axis = "y2") %>%
    dyAxis("y", axisLineColor="red",label = "Delta (µmole/kg)", axisLabelColor ="red", axisLineWidth=5) %>%
    dyAxis("y2", axisLineColor="blue", label = "Value (µmole/kg)",axisLabelColor = "blue", axisLineWidth=5) %>%
    dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
    dyLegend(show = "always")   %>%
    dyOptions(drawGrid = TRUE, drawPoints = TRUE, pointSize = c(1,8,1),useDataTimezone = FALSE, digitsAfterDecimal=2) %>%
    dyRangeSelector(height = 30, dateWindow= NULL) 
...
# CT 50 m
 # plot the differences between duplicates
ct_diff_xts <- dplyr::select(diff, sampling_date, ct_diff, dic_dup)
ct_diff_xts <- as.xts(ct_diff_xts, order.by = diff$sampling_date)
dygraph(ct_diff_xts, main="Differences between CT duplicates 50 m", ylab="CT") %>%
    dySeries("ct_diff", label = "CT delta", color ="red", strokeWidth=0, pointSize=4) %>%
    dySeries("dic_dup", label = "CT duplicates", color = "blue", strokeWidth = 0, pointSize = 2, axis = "y2") %>%
    dyAxis("y", axisLineColor="red",label = "Delta (µmole/kg)", axisLabelColor ="red", axisLineWidth=5) %>%
    dyAxis("y2", axisLineColor="blue", label = "Value (µmole/kg)",axisLabelColor = "blue", axisLineWidth=5) %>%
    dyHighlight(highlightCircleSize = 8,highlightSeriesBackgroundAlpha = 0.5,hideOnMouseOut = TRUE) %>%
    dyLegend(show = "always")   %>%
