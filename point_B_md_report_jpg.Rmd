...
z <- z %>% 
  dplyr::mutate(qflag_ta = ifelse((depth == 0 & ta_dup > 2700) |
                                    (depth == 0 & ta_dup < 2400), 3, qflag_ta))
cols <- c("1" = "red", "2" = "blue", "3" = "darkgreen", "4" = "orange")
CAT1 <- ggplot(dplyr::filter(z, depth == 0), aes(x = sampling_date, y = ta_dup, color = as.factor(qflag_ta))) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(na.rm=TRUE) +
  lims(y = c(2450, 2650)) +
  theme_bw() + 
...
#remove outliers and replot
z <- z %>% 
  #dplyr::mutate(qflag_dic = ifelse(depth == 0 & dic_dup > 2300, 3, qflag_dic)) #this is an indentifed outlier
dplyr::mutate(qflag_dic = ifelse((depth == 0 & dic_dup > 2350) |
  (depth == 0 & dic_dup > 2300 & year(z$sampling_date) == 2009) |
  (depth == 0 & dic_dup > 2300 & year(z$sampling_date) == 2009) |
  (depth == 0 & dic_dup > 2310 & year(z$sampling_date) == 2017) |
  (depth == 0 & dic_dup < 2230 & year(z$sampling_date) == 2014) |
  (depth == 0 & dic_dup < 2210 & year(z$sampling_date) == 2017), 3, qflag_dic))
...
CAT50 <- ggplot(dplyr::filter(z, depth == 50 & qflag_ta != 3), aes(x = sampling_date, y = ta_dup, color = as.factor(qflag_ta))) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(na.rm=TRUE) +
  #lims(y = c(2450, 2650)) +
  theme_bw() + 
  scale_colour_manual(values = c("blue", "red")) +
  labs(y = expression(paste(italic(A)[T]," ","(",mu, mol.kg^-1,")")),x="",
       title="Total alkalinity at 50 m",
       color = "flag")
#remove outliers and replot
z <- z %>%
  dplyr::mutate(qflag_ta = ifelse((depth == 50 & ta_dup < 2520) |
                                  (depth == 50 & ta_dup > 2600), 
                                  3, qflag_ta))
...
CAT1cols <- c("1" = "red", "2" = "blue", "3" = "darkgreen", "4" = "orange")
...
#remove outliers and replot
z <- z %>%
  dplyr::mutate(qflag_dic = ifelse((depth == 50 & dic_dup < 2200) |
        (depth == 50 & dic_dup < 2190) |
        (depth == 50 & dic_dup > 2275 &  year(z$sampling_date) == 2008), 
                                  3, qflag_dic))
