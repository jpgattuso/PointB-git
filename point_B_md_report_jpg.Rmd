...
# add the mean when NA exist in "ta"
tmp <- z %>% 
  ...
  dplyr::mutate(fill = mean(ta, na.rm=TRUE), # value to fill in blanks
  ta_filled_mean = case_when(!is.na(ta) ~ ta,
                    TRUE ~ fill)
  )   
...
pangaea <- dplyr::select(dat, sampling_date, depth, CSC_flag, salinity,
  ...
  dplyr::filter(sampling_date <= as.POSIXct("2020-06-09"))
#Prepare SOMLIT data 
# pH25
pH25 <- dat %>%   
