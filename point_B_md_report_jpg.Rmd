...
cutoff_date <- "2020-01-01" #to limit regresions on complete years
require(readxl)
require(seacarb)
library(cowplot)
library(scales)
...
```{r Calculate mean AT and DIC, include=FALSE}
# Calculate means of duplicate samples and remove duplicates
# one removes  values flagged as 3
z <- z %>%
  ...
  dplyr::ungroup() %>% 
  dplyr::select(-c(sample, sampling_hour, measure_date, ta_dup, dic_dup, pH_ta)) %>% #remove vars not useful and causing problems with distinct
  dplyr::distinct() %>% 
  dplyr::relocate(depth, .before = salinity_field)
```
...
eol <- z %>%
  dplyr::select(sampling_date, depth, ta, dic, salinity_field, temp_field, 
                pH_s_eol_temp_lab, qflag_pH_s_eol, temp_lab_eol, PO4, SIOH4) %>%
  tidyr::drop_na() %>%
  ...
  dplyr::select(-c(pH_s_eol_temp_lab, qflag_pH_s_eol, temp_lab_eol)) %>% # no 
...
ptb <- z %>%
  dplyr::select(sampling_date, depth, ta, dic, salinity_field, temp_field, 
                pH_s_ptb_temp_lab, qflag_pH_s_ptb, temp_lab_ptb, PO4, SIOH4) %>%
  tidyr::drop_na() %>%
  ...
  dplyr::select(-c(pH_s_ptb_temp_lab, qflag_pH_s_ptb, temp_lab_ptb)) %>% # no 
  # longer needed and prevents efficient unique()
  dplyr::ungroup() %>% 
  dplyr::distinct()
...
diff_pH <- ggplot(data = eol_ptb, aes(diff_pH)) +
...
z2 <- dplyr::left_join(z, eol_ptb, by = c("sampling_date", "depth")) %>% 
...
NArows <-  z %>%
  dplyr::ungroup() %>% 
  dplyr::filter(is.na(salinity_field & dic & ta & temp_field)) # rows with NAs
...
dat$pH_calc <- carb$pH
dat$CO2 <- carb$CO2
zz <- left_join(z, dat, by = c("sampling_date", "depth"))
dat2 <- dat%>%
      dplyr::ungroup() %>% 
      dplyr::mutate(
              CSC_flag = carb$flag,
              ...
              pH25 = pHinsi(pH=dat$pH_insi,ALK=dat$ta*1e-6,Tinsi=25,
...
#   dplyr::select(-tidyr::contains(c("eol", "ptb"))) %>% 
#   dplyr::distinct()
...
#             ta_nocarb=mean(ta - HCO3*1e6 - 2*CO3*1e6, na.rm=TRUE),
#             Nta=mean(Nta,na.rm=TRUE),
#             dic=mean(Ndic,na.rm=TRUE),
...
write.table(means_depth, paste0("output_jp/data/output_somlit.csv"), sep= ",", dec=".", row.names = FALSE)
...
mean0m_full <- dplyr::filter(means_depth, depth==0, year(sampling_date) < cutoff_date)
mean50m_full <- dplyr::filter(means_depth, depth==50, year(sampling_date) < cutoff_date)
```

```{r Uncertainties, include=FALSE, echo=FALSE}
means_depth_no_NA <- filter(means_depth, !is.na(ta) & !is.na(dic) & !is.na(salinity)  & !is.na(temperature))
  evar1 <-
...
tmp <- errors(flag = 15, var1 = means_depth_no_NA$ta*1e-6, var2 = means_depth_no_NA$dic*1e-6, 
  S = means_depth_no_NA$mean_salinity, T = means_depth_no_NA$temp_field, P = means_depth_no_NA$depth/10, Pt = 1e-6*means_depth_no_NA$mean_Pt, Sit = 1e-6*means_depth_no_NA$mean_Sit,
...
means_month0m <- ungroup(mean0m) %>%
  ...
  Temperature_sd = sd(temp_field, na.rm = TRUE),
  Temperature_n = sum(!is.na(temp_field)),
  Chla = mean(Chla, na.rm = TRUE),
...
T1vp <- ggplot(filter(tmp_long, variable=="temp_field"), aes(x = month, y = value, group=month)) +
...
T1p <- ggplot(mean0m, aes(x = sampling_date, y = temp_field), na.rm=TRUE) +
...
CT1p <- ggplot(mean0m, aes(x = sampling_date, y = dic), na.rm=TRUE) +
  scale_x_datetime(date_breaks="4 year", date_minor_breaks="1 years", labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  labs(title=NULL,x="",y=expression(paste(italic(C)[T], " (",mu, "mol ", kg^-1,")"))) +
  #annotate("text", x =as.POSIXct("2016-01-01"), y= min(mean0m$dic, na.rm=TRUE), 
  #         label="*", colour="blue", size=10, fontface="plain") +
  coord_fixed() +
  Mytheme(size_labs = 8) +
  theme(axis.text.x=element_blank()) +
  #annotation_custom(grob_D_bl)
AT1p <- ggplot(mean0m, aes(x = sampling_date, y = ta), na.rm=TRUE) +
  scale_x_datetime(date_breaks="4 year", date_minor_breaks="1 years", labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  labs(title=NULL,x="",y=expression(paste(italic(A)[T], " (",mu, "mol ", kg^-1,")"))) +
  #annotate("text", x =as.POSIXct("2016-01-01"), y= min(mean0m$ta, na.rm=TRUE), 
...
ATvsCT1p <- ggplot(mean0m, aes(x = sampling_date, y = ta/dic), na.rm=TRUE) +
  scale_x_datetime(date_breaks="4 year", date_minor_breaks="1 years", labels = date_format("%Y")) +
  ...
  annotate("text", x = as.POSIXct("2008-01-01"), 
           y = max(mean0m$ta/mean0m$dic, na.rm=TRUE), 
           label = lm_eqn(lm(ta/dic ~sampling_date, data=mean0m)), parse = TRUE) +
  coord_fixed() +
  Mytheme(size_labs = 8)
...
tmp <- ungroup(mean0m) %>%
  ...
  Temperature_month = mean(temp_field, na.rm = TRUE),
...
ano_month0m <- left_join(ungroup(mean0m), tmp, by = "monthd") %>%
  ...
  Temperature_ano = temp_field - Temperature_month,
...
#  fm <- as.formula(paste0(i, " ~ sampling_date"))
  #fit[[i]] <- lm(fm, data = mean0m)
  lms <- summary(lm(data=d_reg, ta ~ mean_salinity))
  # calculate probability of fstatistic because it cannot be extracted from lms above
  # see http://stats.stackexchange.com/questions/92824/extracting-the-model-p-value-for-a-multiple-regression-in-r
  # slope returned by lm is per second
  prob <- pf(lms$fstatistic[1],lms$fstatistic[2],lms$fstatistic[3],lower.tail=FALSE)
...
# Plot interannual changes
T50p <- ggplot(mean50m, aes(x = sampling_date, y = temp_field), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  scale_y_continuous(limits = c(12.5, 22.6)) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  labs(title=NULL,x="", y="Temp. (°C)") +
  # annotate("text", x =as.POSIXct("2016-01-01"), y= min(mean50m$temp_field, na.rm=TRUE), 
  #          label="*", colour="blue", size=10, fontface="plain") +
  coord_fixed() +
  Mytheme(size_labs = 8) +
  theme(axis.text.x=element_blank()) +
...
#    scale_y_continuous(limits=c(37, 39), breaks=c(37,38,39)) +
  ...
  theme(axis.text.x=element_blank()) +
  #annotation_custom(grob_B)
CT50p <- ggplot(mean50m, aes(x = sampling_date, y = dic), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  labs(title=NULL,x="",y=expression(paste(italic(C)[T], " (",mu, "mol ", kg^-1,")"))) +
...
AT50p <- ggplot(mean50m, aes(x = sampling_date, y = ta), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
...
Oa50p <- ggplot(mean50m, aes(x = sampling_date, y = mean_OmegaAragonite), na.rm=TRUE) +
  ...
  Mytheme(size_labs = 8) +
  #annotation_custom(grob_G)
ATvsCT50p <- ggplot(mean50m, aes(x = sampling_date, y = ta/dic), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) +
  geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  labs(title=NULL,x="",y=expression(paste(A[T]:C[T]))) +
...
ATnoC1p <- ggplot(mean0m, aes(x = sampling_date, y = ta_nocarb), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  ...
  theme(axis.text.x=element_blank()) +
  #annotation_custom(grob_A_bl)
ATnoC50p <- ggplot(mean50m, aes(x = sampling_date, y = ta_nocarb), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
...
ggsave(file="output_jp/figures/fig_paper_at_nocarb.png", g, width=15, height = 17, units = "cm")
#cowplot::ggsave(file="output_jp/figures/at_nocarb.pdf", g, width = 18, height = 21, units = "cm")
```
...
mean0m_month <- mean0m %>%
  ...
  summarise(Temperature = mean(temp_field, na.rm=TRUE),
...
deconv_dat_50m <- tbl_df(
  data.frame(date=as.POSIXct(mean50m$sampling_date), mAT=means_month50m$AT[mean50m$monthd]*1e-6, mCT=means_month0m$CT[mean50m$monthd]*1e-6, mS=means_month50m$Salinity[mean50m$monthd], mT=means_month0m$Temperature[mean50m$monthd], P=mean50m$mean_P, Pt=mean50m$mean_Pt, Sit=mean50m$mean_Sit, AT=mean50m$ta*1e-6, CT=mean50m$dic*1e-6, S=mean50m$mean_salinity, T=mean50m$temp_field)
)
...
# construct a dataframe with monthly means for the entire series (mXX) as well as real data (without mrefix m)
deconv_dat_50m <- tbl_df(
  data.frame(date=as.POSIXct(mean50m$sampling_date), mAT=means_month0m$AT[mean50m$monthd]*1e-6, mCT=means_month0m$CT[mean50m$monthd]*1e-6, mS=means_month0m$Salinity[mean50m$monthd], mT=means_month0m$Temperature[mean50m$monthd], P=mean50m$mean_P, Pt=mean50m$mean_Pt, Sit=mean50m$mean_Sit, AT=mean50m$ta*1e-6, CT=mean50m$dic*1e-6, S=mean50m$mean_salinity, T=mean50m$temp_field)
)
...
for (i in unique(tmp$year)[-length(unique(tmp$year))]) {
  ...
  j <- j+1
  print(j)
  p[[j]] <- ggplot(data=filter(tmp, tmp$year==i), aes(x=mean_salinity, y=ta, colour=month)) +
...
lms <- summary(lm(data = mean0m, ta ~ mean_salinity))
prob <- pf(lms$fstatistic[1],lms$fstatistic[2],lms$fstatistic[3],lower.tail=FALSE)
...
stat_contour(aes(colour=..level..), breaks=c(8, 8.1, 8.2)) +
  geom_point(data=mean0m, aes(x = dic, y=ta, col=as.factor(year)), na.rm=TRUE) + 
