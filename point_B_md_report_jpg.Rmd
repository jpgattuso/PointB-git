...
z <- z %>%
  ...
  dplyr::select(-c(sample, ta_dup, dic_dup, pH_ta)) %>% #pH_ta not useful and causes problems with distinct
  dplyr::ungroup() %>% 
  dplyr::distinct() %>% 
...
CPHb <- ggplot(eol_ptb_long, aes(x = sampling_date, y = pH, colour = location)) +
  ...
  facet_grid(depth ~ .)
CPHb
# calculate mean pH when data are available both at eol and point B
eol_ptb <- eol_ptb %>% 
  rowwise() %>%
  dplyr::mutate(ph_insi = mean(c(pH_s_eol_temp_insi, pH_s_ptb_temp_insi), na.rm = TRUE))
# add pH variables to data
eol_ptb <- left_join(z, eol_ptb, by = c("sampling_date", "depth"))
```
