...
cutoff_date <- "2021-01-01" #to limit regresions on complete years
require(readxl)
require(seacarb)
library(cowplot)
library(scales)
...
png(file = "figures_jp/fig_dyf_date_AT.png", 
...
png(file = "figures_jp/fig_dyf_AT_salinity.png", 
    width = 15, height = 10, units = "cm", res = 300)
...
fig_dyf_surf_date_AT <- ggplot(data = dyf_surf, aes(x=date, y=AT)) +
  ...
  stat_smooth(method = lm, na.rm=TRUE)
png(file = "figures_jp/fig_dyf_surf_date_AT.png", 
    width = 15, height = 10, units = "cm", res = 300)
...
ggsave(file = "figures_jp/fig_ta_vs_salinity_0m.png", p1,
...
dyf_surf_salinity_AT <- ggplot(data = dyf_surf, aes(x=salinity, y=AT), na.rm=TRUE) +
  ...
  stat_smooth(method = lm, na.rm=TRUE)
ggsave(file = "figures_jp/dyf_surf_salinity_AT.png", dyf_surf_salinity_AT,
...
ggsave(file = "figures_jp/dyf_surf_salinity_AT_year.png", dyf_surf_salinity_AT_year,
...
![figures/dyf_surf_salinity_AT_year.png](figures_jp/dyf_surf_salinity_AT_year.png)
...
z <-  read_delim(
  paste0(path, "data/Data_DIC_tmp.csv"),
  delim = ",",
  col_names = TRUE,
...
# Exclude data for 2019 and later
#z <- filter(z, sampling_date < "2021-01-01")
...
CAT1 <- ggplot(dplyr::filter(z, depth == 0 & qflag_ta != 3), aes(x = sampling_date, y = ta_dup, color = as.factor(qflag_ta))) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(na.rm=TRUE) +
  theme_bw() + 
  scale_colour_manual(values = c("red", "blue")) +
  labs(y = expression(paste(italic(A)[T]," ","(",mu, mol.kg^-1,")")),x="",
       title="Total alkalinity at 1 m",
       color = "flag")
#remove outliers and replot
z <- z %>% 
  dplyr::mutate(qflag_ta = ifelse(depth == 0 & ta_dup > 2700, NA, #these are identifed outliers
                            ifelse(depth == 0 & ta_dup < 2400, 3, qflag_ta)))
...
CAT1 <- ggplot(dplyr::filter(z, depth == 0 & qflag_ta != 3), aes(x = sampling_date, y = ta_dup, color = as.factor(qflag_ta))) +
  ...
  scale_colour_manual(values = c("red", "blue")) +
  labs(y = expression(paste(italic(A)[T]," ","(",mu, mol.kg^-1,")")),x="",
       title="Total alkalinity at 1 m, 3 outliers removed",
       color = "flag")

CCT1 <- ggplot(dplyr::filter(z, depth == 0), aes(x = sampling_date, y = dic, color = as.factor(qflag_dic))) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(na.rm=TRUE) +
  theme_bw() + 
  scale_colour_manual(values = cols) +
  labs(y = expression(paste(italic(C)[T]," ","(",mu, mol.kg^-1,")")),x="",
       title="Dissolved inorganic carbon at 1 m, 3 outliers removed",
       color = "flag")
#remove outliers and replot
z <- z %>% 
  dplyr::mutate(qflag_dic = ifelse(depth == 0 & dic_dup > 2300, NA, qflag_dic)) #this is an indentifed outlier
...
CAT50 <- ggplot(dplyr::filter(z, depth == 0 & qflag_ta != 3), aes(x = sampling_date, y = ta_dup, color = as.factor(qflag_ta))) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(na.rm=TRUE) +
  theme_bw() + 
  scale_colour_manual(values = c("red", "blue")) +
  labs(y = expression(paste(italic(A)[T]," ","(",mu, mol.kg^-1,")")),x="",
       title="Total alkalinity at 50 m, 3 outliers removed",
       color = "flag")
#remove outliers and replot
z <- z %>% 
  dplyr::mutate(qflag_ta = ifelse(depth == 50 & ta_dup < 2500, 3, qflag_ta))
CAT50 <- ggplot(dplyr::filter(z, depth == 50 & qflag_ta != 3), 
...
cols <- c("1" = "red", "2" = "blue", "3" = "darkgreen", "4" = "orange")
CCT50 <- ggplot(dplyr::filter(z, depth == 50 & qflag_dic != 3), 
...
png(file = "figures_jp/fig_outliers.png",
...
# Calculate means of duplicate samples and remove duplicates
# one removes the ta and dic values flagged as 3
z <- z %>%
  dplyr::group_by(sampling_date, depth) %>%
  dplyr::mutate(ta = ifelse(qflag_ta != 3, mean(ta_dup, na.rm = TRUE), NA),
                dic = ifelse(qflag_dic != 3, mean(dic_dup, na.rm = TRUE), NA)) %>% 
  dplyr::select(-c(ta_dup, dic_dup, pH_ta)) %>% #pH_ta not useful and causes problems with distinct
  dplyr::distinct()
...
#save data
write_csv(dat, path="output/output.csv")
write_csv(pangaea, path="output/output_pangaea.csv")
write_rds(dat, "output/output.rds")
```
...
write.table(means_depth, paste0("../output/output_somlit.csv"), sep= ",", dec=".", row.names = FALSE)
...
#cowplot::
ggsave(file="figures_jp/fig4.pdf", g,  width = 8, height = 28, units = "cm")
g <- plot_grid(T1vp, S1vp, AT1vp, CT1vp, pH1vp, pCO21vp, Oa1vp, ncol=2)
ggsave(file="figures_jp/fig_paper_monthly_means_1m.png", g, width = 18, height = 14, units = "cm")
...
![fig_paper_monthly_means_1m.png](figures_jp/fig_paper_monthly_means_1m.png)
...
ATvsCT1p <- ggplot(mean0m, aes(x = sampling_date, y = mean_ta/mean_dic), na.rm=TRUE) +
  ...
  coord_fixed() +
  Mytheme(size_labs = 8)
ggsave(filename = "figures_jp/ATvsCT1p.png", plot = ATvsCT1p)
...
g <- plot_grid(T1p, anT1p, S1p, anS1p, AT1p, anAT1p, CT1p, anCT1p, pH1p, anpH1p, anpHeol1p, anpHptb1p, pCO21p, anpCO21p, Oa1p, anOa1p, align='vh', ncol=2)
ggsave(file="figures_jp/fig_paper_interannual_1m.png", g, width=15, height = 17, units = "cm")
ggsave(file="figures_jp/fig2.pdf", g, width = 18, height = 21, units = "cm")

# AT as a function of salinity for every year
j <- 0
reg <- data.frame()
for (i in 1:(length(unique(mean0m$year))-1)) { #one loops through all years except the last
  j <- j + 1
...
names(reg) <- c("Slope", "SE Slope", "P Slope","Intercept","SE int.","P int.", "F", "df",
                "R2","P value")
row.names(reg) <- as.character(unique(mean0m$year)[-length(unique(mean0m$year))]) # remove last year
#transform in latex table and save
...
fig_poc_pon <- ggplot(data = filter(pom_l, depth==1), aes(x = date, y = value), na.rm=TRUE) + 
  ...
  Mytheme()
ggsave(file="figures_jp/fig_poc_pon.png", fig_poc_pon, width = 18, height = 20, units = "cm")
```
...
* The change in temperature is `r round(reg_var_1m[2, 1], digits=3)`± `r round(reg_var_1m[2,2], digits=3)` °C per year. 
* The change in pH is `r 1000*round(reg_var_1m[7,1], digits=4)` ± `r 1000*round(reg_var_1m[7,2], digits=4)` mpH units per year. If the observed total alkalinity data are corrected from their rate of change (i.e. assuming that they have not increased during the period of measurement, but kept their subannual variability), the rate of change is `r 1000*round(reg_ta_no_change[1,1], digits=4)` ± `r 1000*round(reg_ta_no_change[1,2], digits=4)` mpH units per year.

## Interannual changes at 1 m
![fig_paper_interannual_1m.png](figures_jp/fig_paper_interannual_1m.png)
![fig_poc_pon.png](figures_jp/fig_poc_pon.png)
![fig_ta_vs_salinity_month_pointB_0m.png](figures_jp/fig_ta_vs_salinity_month_pointB_0m.png)
...
ATvsCT50p <- ggplot(mean50m, aes(x = sampling_date, y = mean_ta/mean_dic), na.rm=TRUE) +
  ...
  coord_fixed() +
  Mytheme(size_labs = 8)
ggsave(filename = "figures_jp/ATvsCT50p.png", plot = ATvsCT50p)
...
g <- plot_grid(T50p, anT50p, S50p, anS50p, AT50p, anAT50p, CT50p, anCT50p, pH50p, anpH50p, anpHptb50p, pCO250p, anpCO250p, Oa50p, anOa50p, align='vh', ncol=2)
ggsave(file="figures_jp/fig_paper_interannual_50m.png", g, width=15, height = 17, units = "cm")
ggsave(file="figures_jp/fig3.pdf", g, width = 18, height = 21, units = "cm")
```
## Interannual changes at 50 m
![fig_paper_interannual_50m.png](figures_jp/fig_paper_interannual_50m.png)
...
ATnoC50p <- ggplot(mean50m, aes(x = sampling_date, y = mean_ta_nocarb), na.rm=TRUE) +
  ...
  theme(axis.text.x=element_blank()) +
  annotation_custom(grob_B_bl)
g <- plot_grid(ATnoC1p, ATnoC50p, align='vh', ncol=1)
ggsave(file="figures_jp/fig_paper_at_nocarb.png", g, width=15, height = 17, units = "cm")
#cowplot::ggsave(file="figures_jp/at_nocarb.pdf", g, width = 18, height = 21, units = "cm")
```
...
ggsave(file="figures_jp/fig_temp_month.png", fig_temp_month, width = 15, height = 10, units = "cm")
fig_at_month <- ggplot(data=mean0m_month, aes(x=year, y=at)) + 
  ...
  theme(legend.position="top") +
  guides(col = guide_legend(ncol = 12, byrow = TRUE))
ggsave(file="figures_jp/fig_at_month.png", fig_at_month, width = 15, height = 10, units = "cm")
fig_pCO2_month <- ggplot(data=mean0m_month, aes(x=year, y=pCO2)) + 
  ...
  theme(legend.position="top") +
  guides(col = guide_legend(ncol = 12, byrow = TRUE))
ggsave(file="figures_jp/fig_pCO2_month.png", fig_pCO2_month, width = 15, height = 10, units = "cm")
fig_ct_month <- ggplot(data=mean0m_month, aes(x=year, y=ct)) + 
  ...
  theme(legend.position="top") +
  guides(col = guide_legend(ncol = 12, byrow = TRUE))
ggsave(file="figures_jp/fig_ct_month.png", fig_ct_month, width = 15, height = 10, units = "cm")
...
ggsave(file="figures_jp/fig_paper_monthly_rate_AT_CT.png", fig_paper_monthly_rate_AT_CT, width = 8, height = 8*2/3, units = "cm")
ggsave(file="figures_jp/fig5.pdf", fig_paper_monthly_rate_AT_CT, width = 16, height= 10, units = "cm")
...
fig_temp_quarter <- ggplot(data=mean0m_quart, aes(x=year, y=Temperature)) + 
  ...
  theme(legend.position="top") +
  guides(col = guide_legend(ncol = 12, byrow = TRUE))
ggsave(file="figures_jp/fig_temp_quarter.png", fig_temp_quarter, width = 15, height = 10, units = "cm")
...
fig_at_quarter <- ggplot(data=mean0m_quart, aes(x=year, y=at)) + 
  ...
  theme(legend.position="top") +
  guides(col = guide_legend(ncol = 12, byrow = TRUE))
ggsave(file="figures_jp/fig_at_quarter.png", fig_at_quarter, width = 15, height = 10, units = "cm")
```
...
![fig_temp_month.png](figures_jp/fig_temp_month.png)
![fig_at_month.png](figures_jp/fig_at_month.png)
![fig_pCO2_month.png](figures_jp/fig_pCO2_month.png)
![fig_temp_quarter.png](figures_jp/fig_temp_quarter.png)
![fig_at_quarter.png](figures_jp/fig_at_quarter.png)
...
ggsave(file="figures_jp/fig_paper_monthly_rate_AT_CT_50m.png", fig_paper_monthly_rate_AT_CT_50m, width = 8, height = 8*2/3, units = "cm")
ggsave(file="figures_jp/fig4b.pdf", fig_paper_monthly_rate_AT_CT_50m, width = 16, height= 10, units = "cm")
```
...
ggsave(file="figures_jp/fig_deconv_pH_1m.png", fig_deconv_pH_1m, width = 18, height = 20, units = "cm")
...
fig_deconv_pH_1m_ano <- ggplot(ano_deconv_pH_1m_melt) + geom_point(aes(x=date, y=value, color=variable), size=0.5) +
  ...
  Mytheme() +
  scale_colour_discrete(guide="none")
ggsave(file="figures_jp/fig_deconv_pH_1m_ano.png", fig_deconv_pH_1m_ano, width = 18, height = 20, units = "cm")
```
...
![fig_deconv_pH_1m.png](figures_jp/fig_deconv_pH_1m.png)
...
![fig_deconv_pH_1m_ano.png](figures_jp/fig_deconv_pH_1m_ano.png)
...
ggsave(file="figures_jp/fig_dpH_T_y_dT_1m.png", fig_dpH_T_dT_y_1m, width = 18, height = 12, units = "cm")
...
fig_dT_dt <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dT_dt_1m.png", fig_dT_dt, width = 18, height = 12, units = "cm")
...
fig_dpH_S_dS_y_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_S_dS_y_1m.png", fig_dpH_S_dS_y_1m, width = 18, height = 12, units = "cm")
...
fig_dS_dt_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dS_dt_1m.png", fig_dS_dt_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_AT_dAT_y_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_AT_dT_y_1m.png", fig_dpH_AT_dAT_y_1m, width = 18, height = 12, units = "cm")
...
fig_dAT_dt_1m <- ggreg(fit) +
  labs(x="Year", y="AT") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dAT_dt_1m.png", fig_dAT_dt_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_CT_dCT_y_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_CT_y_dT_1m.png", fig_dpH_CT_dCT_y_1m, width = 18, height = 12, units = "cm")
...
fig_dCT_dt_1m <- ggreg(fit) +
  labs(x="Year", y="CT") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dCT_dt_1m.png", fig_dCT_dt_1m, width = 18, height = 12, units = "cm")
...
![fig_dpH_CT_dCT_y_1m.png](figures_jp/fig_dpH_T_y_dT_1m.png)
![fig_dT_dt_1m.png](figures_jp/fig_dT_dt_1m.png)
![fig_dpH_S_dS_y_1m.png](figures_jp/fig_dpH_S_dS_y_1m.png)
![fig_dS_dt_1m.png](figures_jp/fig_dS_dt_1m.png)
![fig_dpH_AT_dT_y_1m.png](figures_jp/fig_dpH_AT_dT_y_1m.png)

![fig_dAT_dt_1m.png](figures_jp/fig_dAT_dt_1m.png)




![fig_dpH_CT_y_dT_1m.png](figures_jp/fig_dpH_CT_y_dT_1m.png)
![fig_dCT_dt_1m.png](figures_jp/fig_dCT_dt_1m.png)
...
fig_dpH_T_dT_ym_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_T_ym_dT_1m.png", fig_dpH_T_dT_ym_1m, width = 18, height = 12, units = "cm")
...
fig_dT_dt_1m <- ggreg(fit) +
  labs(x="Year", y="Temperature") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dT_dt_1m.png", fig_dT_dt_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_S_dS_ym_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_S_dS_ym_1m.png", fig_dpH_S_dS_ym_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_AT_dAT_ym_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_AT_dT_ym_1m.png", fig_dpH_AT_dAT_ym_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_CT_dCT_ym_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_CT_ym_dT_1m.png", fig_dpH_CT_dCT_ym_1m, width = 18, height = 12, units = "cm")
...
![fig_dpH_T_dT_ym_1m.png](figures_jp/fig_dpH_T_ym_dT_1m.png)
![fig_dT_dt_1m.png](figures_jp/fig_dT_dt_1m.png)
![fig_dpH_S_dS_ym_1m.png](figures_jp/fig_dpH_S_dS_ym_1m.png)
![fig_dS_dt_1m.png](figures_jp/fig_dS_dt_1m.png)
![fig_dpH_AT_dT_ym_1m.png](figures_jp/fig_dpH_AT_dT_ym_1m.png)
![fig_dAT_dt_1m.png](figures_jp/fig_dAT_dt_1m.png)
![fig_dpH_CT_ym_dT_1m.png](figures_jp/fig_dpH_CT_ym_dT_1m.png)
![fig_dCT_dt_1m.png](figures_jp/fig_dCT_dt_1m.png)
...
fig_dpH_T_dT_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_T_t_dT_1m.png", fig_dpH_T_dT_t_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_S_dS_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_S_dS_t_1m.png", fig_dpH_S_dS_t_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_AT_dAT_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_AT_dT_t_1m.png", fig_dpH_AT_dAT_t_1m, width = 18, height = 12, units = "cm")
...
fig_dpH_CT_dCT_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_CT_t_dT_1m.png", fig_dpH_CT_dCT_t_1m, width = 18, height = 12, units = "cm")
...
![fig_dpH_T_t_dT_1m.png](figures_jp/fig_dpH_T_t_dT_1m.png)
![fig_dT_dt_1m.png](figures_jp/fig_dT_dt_1m.png)
![fig_dpH_S_dS_t_1m.png](figures_jp/fig_dpH_S_dS_t_1m.png)
![fig_dS_dt_1m.png](figures_jp/fig_dS_dt_1m.png)
![fig_dpH_AT_dT_t_1m.png](figures_jp/fig_dpH_AT_dT_t_1m.png)
![fig_dAT_dt_1m.png](figures_jp/fig_dAT_dt_1m.png)
![fig_dpH_CT_t_dT_1m.png](figures_jp/fig_dpH_CT_t_dT_1m.png)
![fig_dCT_dt_1m.png](figures_jp/fig_dCT_dt_1m.png)
...
#d <- NULL; deconv <- NULL # d is for the data to use in carb, deconv is to store the results
deconv_pCO2_1m <- tibble(date=as.POSIXct(mean0m$sampling_date))
...
fig_deconv_pCO2_1m <- ggplot(deconv_pCO2_1m_melt) + geom_point(aes(x=date, y=value, color=variable), size=0.5) +
  ...
  Mytheme() +
  scale_colour_discrete(guide="none")
ggsave(file="figures_jp/fig_deconv_pCO2_1m.png", fig_deconv_pCO2_1m, width = 18, height = 20, units = "cm")
...
fig_deconv_pCO2_1m_ano <- ggplot(ano_deconv_pCO2_1m_melt) + geom_point(aes(x=date, y=value, color=variable), size=0.5) +
  ...
  Mytheme() +
  scale_colour_discrete(guide="none")
ggsave(file="figures_jp/fig_deconv_pCO2_1m_ano.png", fig_deconv_pCO2_1m_ano, width = 18, height = 20, units = "cm")
```
Below is the deconvolution performed on values. Here, dvar is the monthly mean value of T, S and CT for the entire time-series.

![fig_deconv_pCO2_1m.png](figures_jp/fig_deconv_pCO2_1m.png)
Below is the deconvolution performed on anomalies Here, dvar is the monthly mean value of the anomalies of T, S and CT for the entire time-series.

![fig_deconv_pCO2_1m_ano.png](figures_jp/fig_deconv_pCO2_1m_ano.png)
...
fig_dpCO2_T_dT_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_T_t_dT_1m.png", fig_dpCO2_T_dT_t_1m, width = 18, height = 12, units = "cm")
...
fig_dpCO2_S_dS_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_S_dS_t_1m.png", fig_dpCO2_S_dS_t_1m, width = 18, height = 12, units = "cm")
...
fig_dpCO2_AT_dAT_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_AT_dT_t_1m.png", fig_dpCO2_AT_dAT_t_1m, width = 18, height = 12, units = "cm")
...
fig_dpCO2_CT_dCT_t_1m <- ggreg(fit) +
  ...
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_CT_t_dT_1m.png", fig_dpCO2_CT_dCT_t_1m, width = 18, height = 12, units = "cm")
...
deconv_pH_50m <- tibble(date=as.POSIXct(mean50m$sampling_date))
...
fig_deconv_pH_50m <- ggplot(deconv_pH_50m_melt) + geom_point(aes(x=date, y=value, color=variable), size=0.5) +
  ...
  Mytheme() +
  scale_colour_discrete(guide="none")
ggsave(file="figures_jp/fig_deconv_pH_50m.png", fig_deconv_pH_50m, width = 18, height = 20, units = "cm")
...
fig_deconv_pH_ano_50m <- ggplot(ano_deconv_pH_50m_melt) + geom_point(aes(x=date, y=value, color=variable), size=0.5) +
  ...
  Mytheme() +
  scale_colour_discrete(guide="none")
ggsave(file="figures_jp/fig_deconv_pH_ano_50m.png", fig_deconv_pH_ano_50m, width = 18, height = 20, units = "cm")
```
...
![fig_deconv_pH_50m.png](figures_jp/fig_deconv_pH_50m.png)
...
![fig_deconv_pH_ano_50m.png](figures_jp/fig_deconv_pH_ano_50m.png)
...
#d <- NULL; deconv_50m <- NULL # d is for the data to use in carb, deconv is to store the results
deconv_pCO2_50m <- tibble(date=as.POSIXct(mean50m$sampling_date))
d <- filter(deconv_dat_50m, !is.na(mAT), !is.na(mCT), !is.na(mS), !is.na(T)) # we eliminate rows with NAs
...
fig_deconv_pCO2_50m <- ggplot(deconv_pCO2_50m_melt) + geom_point(aes(x=date, y=value, color=variable), size=0.5) +
  ...
  Mytheme() +
  scale_colour_discrete(guide="none")
ggsave(file="figures_jp/fig_deconv_pCO2_50m.png", fig_deconv_pCO2_50m, width = 18, height = 20, units = "cm")
...
fig_deconv_pCO2_50m_ano <- ggplot(ano_deconv_pCO2_50m_melt) + geom_point(aes(x=date, y=value, color=variable), size=0.5) +
  ...
  Mytheme() +
  scale_colour_discrete(guide="none")
ggsave(file="figures_jp/fig_deconv_pCO2_50m_ano.png", fig_deconv_pCO2_50m_ano, width = 18, height = 20, units = "cm")
```
Below is the deconvolution performed on values. Here, dvar is the monthly mean value of T, S and CT for the entire time-series.

![fig_deconv_pCO2_50m.png](figures_jp/fig_deconv_pCO2_50m.png)
Below is the deconvolution performed on anomalies Here, dvar is the monthly mean value of the anomalies of T, S and CT for the entire time-series.

![fig_deconv_pCO2_50m_ano.png](figures_jp/fig_deconv_pCO2_50m_ano.png)
...
fig_dpH_T_dT_t_50m <- ggreg(fit) +
  labs(x="Temperature (total mean)", y="pH") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_T_t_dT_50m.png", fig_dpH_T_dT_t_50m, width = 18, height = 12, units = "cm")

fit <- lm(data = deconv_garcia_dat_pH_50m, T ~ sampling_date)
fig_dT_dt_50m <- ggreg(fit) +
  labs(x="Year", y="Temperature") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dT_dt_50m.png", fig_dT_dt_50m, width = 18, height = 12, units = "cm")
...
fig_dpH_S_dS_t_50m <- ggreg(fit) +
  labs(x="Salinity (total mean)", y="pH") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_S_dS_t_50m.png", fig_dpH_S_dS_t_50m, width = 18, height = 12, units = "cm")

fit <- lm(data = deconv_garcia_dat_pH_50m, S ~ sampling_date)
fig_dS_dt_50m <- ggreg(fit) +
  labs(x="Year", y="Salinity") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dS_dt_50m.png", fig_dS_dt_50m, width = 18, height = 12, units = "cm")
...
fig_dpH_AT_dAT_t_50m <- ggreg(fit) +
  labs(x="AT (total mean)", y="pH") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_AT_dT_t_50m.png", fig_dpH_AT_dAT_t_50m, width = 18, height = 12, units = "cm")
...
fig_dAT_dt_50m <- ggreg(fit) +
  labs(x="Year", y="AT") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dAT_dt_50m.png", fig_dAT_dt_50m, width = 18, height = 12, units = "cm")
...
fig_dpH_CT_dCT_t_50m <- ggreg(fit) +
  labs(x="CT (total mean)", y="pH") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpH_CT_t_dT_50m.png", fig_dpH_CT_dCT_t_50m, width = 18, height = 12, units = "cm")
...
fig_dCT_dt_50m <- ggreg(fit) +
  labs(x="Year", y="CT") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dCT_dt_50m.png", fig_dCT_dt_50m, width = 18, height = 12, units = "cm")
...
![fig_dpH_T_t_dT_50m.png](figures_jp/fig_dpH_T_t_dT_50m.png)
![fig_dT_dt_50m.png](figures_jp/fig_dT_dt_50m.png)
![fig_dpH_S_dS_t_50m.png](figures_jp/fig_dpH_S_dS_t_50m.png)
![fig_dS_dt_50m.png](figures_jp/fig_dS_dt_50m.png)
![fig_dpH_AT_dT_t_50m.png](figures_jp/fig_dpH_AT_dT_t_50m.png)
![fig_dAT_dt_50m.png](figures_jp/fig_dAT_dt_50m.png)
![fig_dpH_CT_t_dT_50m.png](figures_jp/fig_dpH_CT_t_dT_50m.png)
![fig_dCT_dt_50m.png](figures_jp/fig_dCT_dt_50m.png)
...
fig_dpCO2_T_dT_t_50m <- ggreg(fit) +
  labs(x="Temperature (total mean)", y="pCO2") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_T_t_dT_50m.png", fig_dpCO2_T_dT_t_50m, width = 18, height = 12, units = "cm")
...
fig_dpCO2_S_dS_t_50m <- ggreg(fit) +
  labs(x="Salinity (total mean)", y="pCO2") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_S_dS_t_50m.png", fig_dpCO2_S_dS_t_50m, width = 18, height = 12, units = "cm")
...
fig_dpCO2_AT_dAT_t_50m <- ggreg(fit) +
  labs(x="AT (total mean)", y="pCO2") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_AT_dT_t_50m.png", fig_dpCO2_AT_dAT_t_50m, width = 18, height = 12, units = "cm")
...
fig_dpCO2_CT_dCT_t_50m <- ggreg(fit) +
  labs(x="CT (total mean)", y="pCO2") +
  Mytheme(size_labs = 10)
ggsave(file="figures_jp/fig_dpCO2_CT_t_dT_50m.png", fig_dpCO2_CT_dCT_t_50m, width = 18, height = 12, units = "cm")
...
p1 <- ggplot(data=dplyr::filter(means_depth, depth== 0)) +
  ...
  Mytheme()
png(file = "figures_jp/fig_ta_vs_salinity_month_pointB_0m.png", 
...
Mytheme_facet()
ggsave(p1, file = "figures_jp/fig_ta_vs_salinity_year_pointB_0m.png", 
    width = 6, height = 6)

# AT versus salinity at the surface per year with loop
panel <- c("(a)","(b)","(c)","(d)","(e)","(f)","(g)","(h)","(i)", "(j)", "(k)", "(l)", "(m)")
tmp <- filter(means_depth, depth== 0, sampling_date < cutoff_date)
p <- NULL; j <- 0
...
g1 <- plot_grid(p[[1]], p[[2]], p[[3]], p[[4]], p[[5]], p[[6]], p[[7]], p[[8]], 
                p[[9]], p[[10]], p[[11]], p[[12]], p[[13]], ncol=3, align="vh")
...
ggsave(file="figures_jp/fig6.pdf", g, width = 16, height = 15, units = "cm")
...
p1 <- ggplot(data=mean0m) +
  ...
  Mytheme()
ggsave(p1, filename = "figures_jp/fig_ta_vs_salinity_all_pointB_0m.png", width = 20, height = 13, units = "cm")
...
p1 <- ggplot(data=means_month0m) +
  ...
  Mytheme()
png(file = "figures_jp/fig_ta_vs_salinity_monthly_means_pointB_0m.png", 
    width = 20, height = 13, units = "cm", res = 300)
print(p1)
dev.off()
...
ggsave("figures_jp/precip.png",g, width = 18, height = 14, units = "cm")
ggsave("figures_jp/figS2.pdf",g, width = 8, height = 14, units = "cm")
```

The relationship between AT and salinity using all data is shown below.
![fig_ta_vs_salinity_all_pointB_0m.png](figures_jp/fig_ta_vs_salinity_all_pointB_0m.png)

The relationship between AT and salinity using monthly means is shown below.
![fig_ta_vs_salinity_monthly_means_pointB_0m.png](figures_jp/fig_ta_vs_salinity_monthly_means_pointB_0m.png)
...
![precip.png](figures_jp/precip.png)


## Daily and monthly changes in pH and T
```{r Daily and seasonal changes, include=FALSE}
# Long series
continuous_discrete <- read_csv(file = "../data/continuous_discrete.csv", 
                                col_names = TRUE,
...
plot_range_pH_Chla_month <-  ggplot(data = range_month) +
  ...
  theme_bw()
ggsave("figures_jp/plot_range_pH_Chla_month.png", plot_range_pH_Chla_month)
...
plot_range_pH_Temperature <-  ggplot(data = range_month) +
  ...
  annotation_custom(grob_A)
ggsave("figures_jp/plot_range_pH_Temperature.png", plot_range_pH_Temperature)
...
ggsave("figures_jp/fig_paper_time_series.png", g, width = 18, height = 14, units = "cm")
ggsave(file="figures_jp/fig7.pdf", g, width = 8, height = 16, units = "cm")
...
plot_range_pH_Chla_week <-  ggplot(data =  range_year_week) +
  ...
  theme(aspect.ratio=1/2,
        axis.title.x = element_text(margin=margin(t=0.25, r=0, b=0.25, l=0, unit = "cm"))) +
  annotation_custom(grob_B)
ggsave("figures_jp/plot_range_pH_Chla_week.png", plot_range_pH_Chla_week)

plot_range_pH_Chla_temp <- plot_grid(plot_range_pH_Temperature,
                                     plot_range_pH_Chla_week, ncol = 1, align = "h")
ggsave("figures_jp/plot_range_pH_Chla_temp.png", plot_range_pH_Chla_temp, width = 8)
ggsave("figures_jp/figS1.pdf", plot_range_pH_Chla_temp, width = 8, height = 8)

```

The average error in pH due to spatiotemporal mismatch between sensor and discrete sample is `r average_offset`.
![fig_paper_time_series.png](figures_jp/fig_paper_time_series.png)
...
![plot_range_pH_Chla_month.png](figures_jp/plot_range_pH_Chla_month.png)
![plot_range_pH_Chla_temp.png](figures_jp/plot_range_pH_Chla_temp.png)
...
ggsave("figures_jp/fig_atm_CO2.png", g)
```
...
![fig_atm_CO2.png](figures_jp/fig_atm_CO2.png)
...
deff_monthly <- ggplot() +
  ...
  ggsave(file = "figures_jp/fig_deffayes_monthly.png", deff_monthly,
...
deff_yearly <- ggplot() +
  ...
  labs(title="Point", 
       ...
       y=expression(paste(italic(A)[T]," ","(",mu, mol.kg^-1,")"))) +
  scale_color_viridis(name = "Month", discrete = TRUE, option = "plasma")
  #ggsave(file = "figures_jp/fig_deffayes_monthly.png", deff_monthly, width = 15, height = 15, units = "cm")
...
![fig_deffayes_monthly.png](figures_jp/fig_deffayes_monthly.png)
