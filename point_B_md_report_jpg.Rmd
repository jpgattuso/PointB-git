...
eol <- tmp %>%
  ...
  tidyr::drop_na(sampling_date, depth, ta, salinity_field, temp_field, 
                pH_s_eol_temp_lab, qflag_pH_s_eol, temp_lab_eol) %>%
...
ptb <- tmp %>%
  ...
  tidyr::drop_na(sampling_date, depth, ta, salinity_field, temp_field, 
                pH_s_ptb_temp_lab, qflag_pH_s_ptb, temp_lab_ptb) %>%
...
# Now pH25 (must be distinct to above because NAs are not the same)
tmp <- dat2 %>% 
  dplyr::select(sampling_date, depth, salinity, temperature, pH_insi, ta, Pt, Sit) %>% 
  tidyr::drop_na(salinity, temperature, ta, pH_insi)
pHat25 <- seacarb::pHinsi(pH=tmp$pH_insi,ALK=tmp$ta*1e-6,Tinsi=25,
                            Tlab=tmp$temperature,S=tmp$salinity,
                            Pt=1e-6*tmp$Pt, Sit=1e-6*tmp$Sit,
                            k1k2 = "l",
                            kf = "pf", pHscale = "T")
tmp <- tmp %>% dplyr::mutate(pH25 = pHat25) %>% 
    dplyr::select(sampling_date, depth, pH25)
...
dat_month0m <- ungroup(dat0m) %>%
  ...
  ta_n = sum(!is.na(ta)),
  pH_insi = mean(pH_insi, na.rm = TRUE),
  pH_sd = sd(pH_insi, na.rm = TRUE),
  pH_n = sum(!is.na(pH_insi)),
  pH25 = mean(pH25, na.rm = TRUE),
  pH25_sd = sd(pH25, na.rm = TRUE),
  pH25_n = sum(!is.na(pH25)),
...
pH1vp <- ggplot(filter(tmp_long, variable=="pH_insi"), aes(x = month, y = value, group=month)) +
  geom_violin(fill='lightblue', alpha=0.5, size=0.2, trim = TRUE) +
  geom_boxplot(notch=TRUE, size=0.2, fill="grey50", outlier.color = "blue", outlier.size = 0.3) +
  scale_x_discrete(breaks=c("Jan", "", "Mar", "", "May", "", "Jul", "", "Sep", "", "Nov", "")) +
...
means_month50m <- ungroup(dat50m) %>%
  ...
  ta_sd = sd(ta, na.rm = TRUE),
  ta_n = sum(!is.na(ta)),
  pH_s_eol_temp_insi = mean(pH_s_eol_temp_insi, na.rm = TRUE),
  pH_s_eol_temp_insi_sd = sd(pH_s_eol_temp_insi, na.rm = TRUE),
  pH_s_eol_temp_insi_n = sum(!is.na(pH_s_eol_temp_insi)),
  pH_s_ptb_temp_insi = mean(pH_s_ptb_temp_insi, na.rm = TRUE),
  pH_s_ptb_temp_insi_sd = sd(pH_s_ptb_temp_insi, na.rm = TRUE),
  pH_s_ptb_temp_insi_n = sum(!is.na(pH_s_ptb_temp_insi)),
...
T1p <- ggplot(dat0m, aes(x = sampling_date, y = temp_field), na.rm=TRUE) +
...
S1p <- ggplot(dat0m, aes(x = sampling_date, y = mean_salinity), na.rm=TRUE) +
  scale_x_datetime(date_breaks="4 year", date_minor_breaks="1 years", labels = date_format("%Y")) +
...
pH1p <- ggplot(dat0m, aes(x = sampling_date, y = mean_pH), na.rm=TRUE) +
  scale_x_datetime(date_breaks="4 year", date_minor_breaks="1 years", labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  geom_point(data=dat0m, aes(x = sampling_date, y = mean_pH_s_eol_temp_insi),color= "blue", size=1) +
 ...
 labs(title=NULL,x="", y=expression(paste(pH[T]))) +
  coord_fixed() +
  Mytheme(size_labs = 8) +
  #annotate("text", x =as.POSIXct("2016-01-01"), y= min(dat0m$mean_pH, na.rm=TRUE), 
  #         label="*", colour="blue", size=9, fontface="plain") +
  theme(axis.text.x=element_blank()) +
  #annotation_custom(grob_E_bl)
pCO21p <- ggplot(dat0m, aes(x = sampling_date, y = mean_pCO2), na.rm=TRUE) +
  scale_x_datetime(date_breaks="4 year", date_minor_breaks="1 years", labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
...
Oa1p <- ggplot(dat0m, aes(x = sampling_date, y = mean_OmegaAragonite), na.rm=TRUE) +
  scale_x_datetime(date_breaks="4 year", date_minor_breaks="1 years", labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
...
# Plot interannual changes
T50p <- ggplot(dat50m, aes(x = sampling_date, y = temp_field), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  ...
  theme(axis.text.x=element_blank()) +
  #annotation_custom(grob_A)
S50p <- ggplot(dat50m, aes(x = sampling_date, y = mean_salinity), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
...
AT50p <- ggplot(dat50m, aes(x = sampling_date, y = ta), na.rm=TRUE) +
  ...
  theme(axis.text.x=element_blank()) +
  #annotation_custom(grob_C)
pH50p <- ggplot(dat50m, aes(x = sampling_date, y = mean_pH), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  geom_point(data=dat50m, aes(x = sampling_date, y = mean_pH_s_ptb_temp_insi), color ="red", size=1) +
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
 labs(title=NULL,x="", y=expression(paste(pH[T]))) +
...
pCO250p <- ggplot(dat50m, aes(x = sampling_date, y = mean_pCO2), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  labs(title=NULL,x="",y=expression(paste(pCO[2], " (",mu, "atm)"))) +
  ...
  theme(axis.text.x=element_blank()) +
  #annotation_custom(grob_F)
Oa50p <- ggplot(dat50m, aes(x = sampling_date, y = mean_OmegaAragonite), na.rm=TRUE) +
  scale_x_datetime(breaks = date_breaks("2 year"), labels = date_format("%Y")) +
  geom_point(colour="black", na.rm=TRUE, size=size_point) + 
  #geom_smooth(method=lm, colour="black", fill="grey", size=0.6, na.rm=TRUE) +
  labs(title=NULL,x="",y=expression(paste(Omega[a]))) +
...
deconv_dat_pH_1m <- tibble(date=as.POSIXct(dat0m$sampling_date), mAT=means_month0m$AT[dat0m$monthd]*1e-6, mCT=means_month0m$CT[dat0m$monthd]*1e-6, mS=means_month0m$Salinity[dat0m$monthd], mT=means_month0m$Temperature[dat0m$monthd], P=dat0m$mean_P, Pt=dat0m$mean_Pt, Sit=dat0m$mean_Sit, AT=dat0m$ta*1e-6, CT=dat0m$dic*1e-6, S=dat0m$mean_salinity, T=dat0m$temp_field)
...
deconv_dat_pCO2_1m <- tbl_df(
  data.frame(date=as.POSIXct(dat0m$sampling_date), mAT=means_month0m$AT[dat0m$monthd]*1e-6, mCT=means_month0m$CT[dat0m$monthd]*1e-6, mS=means_month0m$Salinity[dat0m$monthd], mT=means_month0m$Temperature[dat0m$monthd], P=dat0m$mean_P, Pt=dat0m$mean_Pt, Sit=dat0m$mean_Sit, AT=dat0m$ta*1e-6, CT=dat0m$dic*1e-6, S=dat0m$mean_salinity, T=dat0m$temp_field)
)
...
deconv_dat_50m <- tbl_df(
  data.frame(date=as.POSIXct(dat50m$sampling_date), mAT=means_month50m$AT[dat50m$monthd]*1e-6, mCT=means_month0m$CT[dat50m$monthd]*1e-6, mS=means_month50m$Salinity[dat50m$monthd], mT=means_month0m$Temperature[dat50m$monthd], P=dat50m$mean_P, Pt=dat50m$mean_Pt, Sit=dat50m$mean_Sit, AT=dat50m$ta*1e-6, CT=dat50m$dic*1e-6, S=dat50m$mean_salinity, T=dat50m$temp_field)
)
...
# construct a dataframe with monthly means for the entire series (mXX) as well as real data (without mrefix m)
deconv_dat_50m <- tbl_df(
  data.frame(date=as.POSIXct(dat50m$sampling_date), mAT=means_month0m$AT[dat50m$monthd]*1e-6, mCT=means_month0m$CT[dat50m$monthd]*1e-6, mS=means_month0m$Salinity[dat50m$monthd], mT=means_month0m$Temperature[dat50m$monthd], P=dat50m$mean_P, Pt=dat50m$mean_Pt, Sit=dat50m$mean_Sit, AT=dat50m$ta*1e-6, CT=dat50m$dic*1e-6, S=dat50m$mean_salinity, T=dat50m$temp_field)
)
...
lms <- summary(lm(data = dat0m, ta ~ mean_salinity))
prob <- pf(lms$fstatistic[1],lms$fstatistic[2],lms$fstatistic[3],lower.tail=FALSE)
