...
ggreg2 <- function (fit, xdata, ydata, type = "MA", title = NULL) { # x and y are the names of the variables
  fit_data <- data.frame(xdata, ydata)
...
ggreg(fit = lms, title = "AT vs Salinity")
ggreg(xdata = dat0m$salinity, ydata = dat0m$ta, type = "OLS", plot = TRUE)

ggreg <- function (xdata, ydata, type = "MA", plot = TRUE, title = NULL) { # x and y are the names of the variables
  fit_data <- data.frame(xdata, ydata)
  colnames(fit_data) = c("xdata", "ydata")
  fit <- lmodel2::lmodel2(ydata ~ xdata)
if (type == "OLS") ty = 1 #one selects OLS only
if (type == "MA") ty = 2 #one selects MA only
reg <- fit$regression.results[ty,] #one selects OLS only
intercept <- reg$Intercept
slope <- reg$Slope
if (plot == TRUE) {
  plt <- ggplot(data = fit_data, x = fit_data$xdata, y = fit_data$ydata) + 
  geom_point(size = 2, col = "blue") +
  geom_abline(aes(intercept = fit$regression.results[ty,2], slope = fit$regression.results[ty,3]),
              colour = "blue")  + 
  labs(title = paste(title,"\n Adj R2 = ", signif(fit$rsquare, 3),
...
ydataggreg <- function (fit, point_size=2, title = NULL) {
  ggplot(fit$model, aes_string(x = names(fit$model)[2],
                               y = names(fit$model)[1])) +
    geom_point(size = point_size, col = "blue") +
    stat_smooth(method = "lm", col = "black") +
